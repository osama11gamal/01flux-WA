// API Service Layer for 01flux WA Dashboard
...
export interface InfraStatus {
  // `builtIn` = 01flux WA's own bundled container is actually running and backing this service (live),
...
async function handleErrorResponse<T>(response: Response): Promise<T> {
  if (response.status === 401) {
    sessionStorage.removeItem('flux_wa_api_key');
...
export interface CatalogPlugin {
  ...
  license?: string;
  keywords?: string[];
  minFluxWaVersion?: string;
  testedFluxWaVersion?: string;
  homepage?: string;
  download?: string;
  installed: boolean;
