...
if (process.env.MCP_ENABLED === 'true') {
  ...
  mcpModules.push(
    McpModule.forRoot({
      basePath: '/mcp',
      serverInfo: { name: '01flux-wa', version },
    }),
  );
}
...
if (dashboardServingEnabled && dashboardBuildPresent) {
  serveStaticModules.push(
    ServeStaticModule.forRoot({
      ...
      // contains a dot-segment (~/.01flux-wa, a checkout under ~/.cache): it sends the index by
      ...
      renderPath: '/__flux_wa_spa_fallback_owned_by_main_ts__',
    }),
  );
}
      ...
      useFactory: (configService: ConfigService) => {
        ...
        if (dbType === 'postgres') {
          ...
          return {
            ...
            database: configService.get<string>('dataDatabase.name', '01flux-wa'),
