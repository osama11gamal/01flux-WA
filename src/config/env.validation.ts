...
export function validateEnv(config: EnvConfig): EnvConfig {
  ...
  if (dbType === 'postgres') {
    ...
    // PostgreSQL DATABASE_NAME (e.g. '01flux-wa') leaking into a SQLite run (#677). That bare name becomes
    // the SQLite file PATH, opening a file named '01flux-wa' under the read-only app rootfs →
