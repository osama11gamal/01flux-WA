...
/**
 * The plugin package dir 01flux WA ≤ 0.12.1 defaulted to. A host that ran on that default has working
...
export default () => ({
  ...
  dataDatabase: {
    ...
    // behavior; set POSTGRES_SCHEMA to place 01flux WA's tables + the TypeORM migration ledger in a
    // dedicated schema (e.g. a managed-Postgres project schema, or to isolate 01flux WA from other
  ...
  plugins: {
    ...
    // Remote catalog of installable plugins (JSON array; the 01flux WA-plugins repo's plugins.json).
    // Fetched through the SSRF guard — add its host to SSRF_ALLOWED_HOSTS if it is not publicly resolvable.
    catalogUrl:
      process.env.PLUGIN_CATALOG_URL || 'https://raw.githubusercontent.com/rmyndharis/fluxwa-plugins/main/plugins.json',
