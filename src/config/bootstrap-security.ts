 ...
 * generator emits 71 chars (`flx_k1_` + 32 bytes hex), so a generated key always passes.
 */
...
export interface SecretCheckEnv {
  ...
  /** POSTGRES_BUILTIN — when 'true', 01flux WA runs the bundled Postgres on the internal-only network. */
  ...
  s3Endpoint?: string;
  /** MINIO_BUILTIN — when 'true', 01flux WA runs the bundled MinIO on the internal-only network. */
  minioBuiltIn?: string;
  apiMasterKey?: string;
...
export function assertNoDefaultSecretsInProduction(env: SecretCheckEnv): void {
  ...
  // '01flux-wa'/'minioadmin' credentials are not internet-reachable — exempt them so selecting the
