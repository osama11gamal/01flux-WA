...
describe('assertNoDefaultSecretsInProduction', () => {
  ...
  it('treats an unrecognised NODE_ENV as production rather than skipping the guard', () => {
    ...
    expect(() =>
      assertNoDefaultSecretsInProduction({ nodeEnv: 'staging', databaseType: 'postgres', databasePassword: '01flux-wa' }),
  ...
  it('allows the built-in Postgres/MinIO default credentials in prod (internal-only network) (#488 review)', () => {
    ...
    // known '01flux-wa'/'minioadmin' creds the built-in flow provisions must not crash-loop a prod boot.
    expect(() =>
      assertNoDefaultSecretsInProduction({
        nodeEnv: 'production',
        databaseType: 'postgres',
  ...
  it('allows prod with a strong generated-shape API_MASTER_KEY', () => {
    // The shape the first-boot generator emits: `flx_k1_` + 32 bytes hex (71 chars).
    expect(() =>
      assertNoDefaultSecretsInProduction({ nodeEnv: 'production', apiMasterKey: `flx_k1_${'ab'.repeat(32)}` }),
    ).not.toThrow();
  });
  ...
  it('does not check the DB password when using sqlite', () => {
    ...
    expect(() =>
      assertNoDefaultSecretsInProduction({ nodeEnv: 'production', databaseType: 'sqlite', databasePassword: '01flux-wa' }),
    ).not.toThrow();
  });
