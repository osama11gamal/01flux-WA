...
describe('validateEnv', () => {
  ...
  it('validates POSTGRES_SCHEMA as a legal, non-reserved Postgres identifier when set', () => {
    ...
    expect(() => validateEnv({ ...pg, POSTGRES_SCHEMA: 'fluxwa' })).not.toThrow();
    ...
    expect(() => validateEnv({ ...pg, POSTGRES_SCHEMA: '01flux-wa; DROP' })).toThrow(/POSTGRES_SCHEMA/);
  ...
  it('resolves the main DB path from MAIN_DATABASE_NAME like the runtime (no false-negative/-positive)', () => {
    ...
    expect(() =>
      validateEnv({
        ...
        MAIN_DATABASE_NAME: '/srv/01flux-wa/main.sqlite',
        DATABASE_NAME: '/srv/01flux-wa/main.sqlite',
      }),
  ...
  it('rejects a bare SQLite DATABASE_NAME (PG-name leak) that has no path separator or file extension', () => {
    // Regression for #677: .env.example shipped `DATABASE_NAME=01flux-wa` (a PostgreSQL db name).
    // In a SQLite run that bare name becomes the file PATH → SQLite opens a file named '01flux-wa'
    // under the read-only app rootfs → SQLITE_CANTOPEN boot-loop. The guard catches the leak at boot.
    expect(() => validateEnv({ DATABASE_TYPE: 'sqlite', DATABASE_NAME: '01flux-wa' })).toThrow(/file path/);
