...
/**
 * Prometheus exposition for 01flux WA. Kept dependency-free (no prom-client) — the
...
export class MetricsService {
  ...
  async render(): Promise<string> {
    ...
    gauge('flux_wa_up', 'Whether the 01flux WA process is up (always 1 when scraped).', 1);
    gauge('flux_wa_process_uptime_seconds', 'Process uptime in seconds.', Math.round(process.uptime()));
    gauge('flux_wa_process_resident_memory_bytes', 'Resident set size in bytes.', mem.rss);
    gauge('flux_wa_process_heap_used_bytes', 'V8 heap used in bytes.', mem.heapUsed);

    gauge(
      'flux_wa_stats_available',
      'Whether the database-derived series below could be read on this scrape (1) or not (0).',
      overview ? 1 : 0,
    );

    if (overview) {
      gauge('flux_wa_sessions_total', 'Total number of configured sessions.', overview.sessions.total);
      gauge('flux_wa_sessions_active', 'Number of READY (active) sessions.', overview.sessions.active);

      // Per-status session counts share one metric name with a `status` label.
      lines.push('# HELP flux_wa_sessions Number of sessions by status.');
      lines.push('# TYPE flux_wa_sessions gauge');
      for (const [status, count] of Object.entries(overview.sessions.byStatus)) {
        lines.push(`flux_wa_sessions{status="${this.escapeLabel(status)}"} ${count}`);
      }

      lines.push('# HELP flux_wa_messages_total Current stored messages by direction.');
      lines.push('# TYPE flux_wa_messages_total gauge');
      lines.push(`flux_wa_messages_total{direction="outgoing"} ${overview.messages.sent}`);
      lines.push(`flux_wa_messages_total{direction="incoming"} ${overview.messages.received}`);

      lines.push('# HELP flux_wa_messages_failed_total Current stored messages in FAILED state.');
      lines.push('# TYPE flux_wa_messages_failed_total gauge');
      lines.push(`flux_wa_messages_failed_total ${overview.messages.failed}`);
    }

    lines.push(
      '# HELP flux_wa_webhook_delivery_failures_total Webhook deliveries that terminally failed (all retries exhausted) since process start.',
    );
    lines.push('# TYPE flux_wa_webhook_delivery_failures_total counter');
    lines.push(`flux_wa_webhook_delivery_failures_total ${getWebhookDeliveryFailuresTotal()}`);

    lines.push(
      '# HELP flux_wa_session_reconnect_attempts_total Reconnect attempts scheduled across all sessions since process start.',
    );
    lines.push('# TYPE flux_wa_session_reconnect_attempts_total counter');
    lines.push(`flux_wa_session_reconnect_attempts_total ${getSessionReconnectAttemptsTotal()}`);

    lines.push('# HELP flux_wa_session_reconnect_loop_alerts_total Reconnect-loop alerts emitted since process start.');
    lines.push('# TYPE flux_wa_session_reconnect_loop_alerts_total counter');
    lines.push(`flux_wa_session_reconnect_loop_alerts_total ${getSessionReconnectLoopAlertsTotal()}`);

    lines.push('# HELP flux_wa_sessions_restricted Sessions whose account WhatsApp is currently restricting.');
    lines.push('# TYPE flux_wa_sessions_restricted gauge');
    lines.push(`flux_wa_sessions_restricted ${getRestrictedSessionCount()}`);
    ...
    if (refusals.size > 0) {
      lines.push('# HELP flux_wa_send_pacing_refusals_total Sends refused by the pacing governor since process start.');
      lines.push('# TYPE flux_wa_send_pacing_refusals_total counter');
      for (const [reason, count] of refusals) {
        lines.push(`flux_wa_send_pacing_refusals_total{reason="${this.escapeLabel(reason)}"} ${count}`);
      }
    }
