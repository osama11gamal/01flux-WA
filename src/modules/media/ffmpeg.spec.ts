...
describe('ffmpeg invocation shape', () => {
  const args = buildFfmpegArgs('/tmp/01flux-wa-convert-x/in.bin', '/tmp/01flux-wa-convert-x/out.ogg', ['-c:a', 'libopus']);
  ...
  it('names the input immediately after -i, and the output last', () => {
    expect(args[args.indexOf('-i') + 1]).toBe('/tmp/01flux-wa-convert-x/in.bin');
    expect(args[args.length - 1]).toBe('/tmp/01flux-wa-convert-x/out.ogg');
  });
