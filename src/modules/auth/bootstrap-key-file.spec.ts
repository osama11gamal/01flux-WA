...
describe('bootstrap key file', () => {
  ...
  describe('readBootstrapKey', () => {
    ...
    it('returns the trimmed key', () => {
      writeFileSync(join(dir, '.api-key'), '  flx_k1_abc \n');

      expect(readBootstrapKey(logger)).toBe('flx_k1_abc');
    });
  });

  describe('writeBootstrapKey', () => {
    it('writes to the resolved path', () => {
      writeBootstrapKey('flx_k1_written');

      expect(readFileSync(join(dir, '.api-key'), 'utf-8')).toContain('flx_k1_written');
    });
  });
