...
describe('resolveSeedApiKey (first-boot default admin key)', () => {
  ...
  it('generates a random flx_k1_ key by default (no opt-in)', () => {
    expect(resolveSeedApiKey()).toMatch(/^flx_k1_[a-f0-9]{64}$/);
  });
...
describe('bannerKeyLine (startup banner key masking)', () => {
  const FULL = 'flx_k1_0123456789abcdef0123456789abcdef';
  ...
  it('masks the key on subsequent boots — the full secret is never re-logged', () => {
    ...
    expect(line.startsWith('flx_k1_0')).toBe(true); // a short fingerprint is fine
...
describe('AuthService', () => {
  ...
  describe('createApiKey', () => {
    it('should generate a key with flx_k1_ prefix and save to DB', async () => {
      ...
      expect(result.rawKey).toMatch(/^flx_k1_[a-f0-9]{64}$/);
  ...
  describe('bootstrap API key file (data/.api-key)', () => {
    ...
    it('boot removes a stale bootstrap file and the banner no longer advertises the dead key', async () => {
      ...
      existsSpy.mockReturnValue(true);
      readSpy.mockReturnValue('flx_k1_deaddeaddead');
      ...
      expect(bannerText()).not.toContain('flx_k1_d'); // no fingerprint of the dead key
    });
    ...
    it('boot keeps the bootstrap file when only the pepper changed (prefix matches, hash does not)', async () => {
      ...
      existsSpy.mockReturnValue(true);
      readSpy.mockReturnValue('flx_k1_pepperchanged');
      ...
      (repository.findOne as jest.Mock)
        ...
        .mockResolvedValue(createMockApiKey({ keyPrefix: 'flx_k1_peppe', keyHash: 'hash-under-old-pepper' }));

      await service.onModuleInit();

      expect(repository.findOne).toHaveBeenCalledWith({ where: { keyPrefix: 'flx_k1_peppe' } });
      expect(unlinkSpy).not.toHaveBeenCalled();
    ...
    it('boot still deletes the file when no row carries the file key prefix (genuine staleness)', async () => {
      ...
      expect(repository.findOne).toHaveBeenCalledWith({ where: { keyPrefix: 'flx_k1_deadd' } });
      expect(unlinkSpy).toHaveBeenCalledWith(expect.stringContaining('.api-key'));
    });
  ...
  describe('hashKey reads API_KEY_PEPPER', () => {
    ...
    it('hashes with HMAC-SHA256 when the pepper is set', async () => {
      ...
      const queried = await queriedHash('flx_raw_key');
      expect(queried).toBe(createHmac('sha256', 'server-pepper').update('flx_raw_key').digest('hex'));
      expect(queried).not.toBe(createHash('sha256').update('flx_raw_key').digest('hex'));
    });
    ...
    it('hashes with plain SHA-256 when the pepper is unset (existing keys keep validating)', async () => {
      ...
      const queried = await queriedHash('flx_raw_key');
      expect(queried).toBe(createHash('sha256').update('flx_raw_key').digest('hex'));
    });
  });
});
