...
export function applyDatabaseSection(
  ...
  if (database.type === 'postgres') {
    if (dbBuiltIn) {
      ...
      updates.DATABASE_USERNAME = '01flux-wa';
      // The bundled credential is only the DEFAULT. Secrets are never echoed back to the form,
      // so an absent password field means "unchanged", not "reset to '01flux-wa'" — and the
      ...
      updates.DATABASE_PASSWORD = database.password || storedPassword || '01flux-wa';
      updates.DATABASE_NAME = '01flux-wa';
      ...
      // '01flux-wa' password into the external config: the production boot guard rejects
      ...
      if (database.database !== undefined) updates.DATABASE_NAME = database.database || '01flux-wa';
