...
export class InfraConfigController {
  ...
  private assertProductionBootable(merged: Record<string, string>): void {
    ...
    // external switch keeping the bundled '01flux-wa' password would save cleanly and crash-loop the
  ...
  async requestRestart(@Body() body?: RestartDto): Promise<{
    ...
    if (this.dockerService.isDockerAvailable()) {
      ...
      // Only ever tear down 01flux WA-managed services. An arbitrary profile name (or the empty string)
      ...
      // and drop anything else. (DockerService already hard-prefixes 01flux-wa-<service> and filters on
      // the com.01flux-wa.service label, so this is defense-in-depth, not the sole control.)
