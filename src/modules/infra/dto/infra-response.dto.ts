...
export class InfraDatabaseStatusDto {
  ...
  @ApiProperty({ description: 'Host the app is configured to reach.', example: '01flux-wa-postgres' })
  host!: string;

  @ApiProperty({
    description:
      "Whether 01flux WA's own bundled container is actually running and backing this service, " +
...
export class InfraRedisStatusDto {
  ...
  @ApiProperty({ example: '01flux-wa-redis' })
  host!: string;

  @ApiProperty({ example: 6379 })
  port!: number;

  @ApiProperty({ description: "Whether 01flux WA's bundled Redis container is backing this.", example: true })
  builtIn!: boolean;
}
...
export class InfraStorageStatusDto {
  ...
  @ApiPropertyOptional({ description: 'Bucket name. Present only for `s3`.', example: '01flux-wa-media' })
  bucket?: string;

  @ApiProperty({ description: "Whether 01flux WA's bundled MinIO container is backing this.", example: false })
  builtIn!: boolean;
...
export class InfraConfigDatabaseDto {
  ...
  @ApiProperty({ example: '01flux-wa-postgres' })
  host!: string;

  @ApiProperty({ description: 'Kept as a string: it is echoed back from the saved env verbatim.', example: '5432' })
  port!: string;

  @ApiProperty({ example: '01flux-wa' })
  username!: string;

  @ApiProperty({ description: 'Database name, or the SQLite file path.', example: '01flux-wa' })
  database!: string;
