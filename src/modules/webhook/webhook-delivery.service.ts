...
export class WebhookDeliveryService implements OnModuleInit, OnModuleDestroy {
  ...
  private async preflightDelivery(
    ...
    try {
      ...
      // receiver verifies the signature over this body and compares it against the X-FluxWa-*
      ...
      const headers = {
        ...
        'User-Agent': '01flux WA-Webhook/1.0.0',
        'X-FluxWa-Event': event,
        'X-FluxWa-Idempotency-Key': idempotencyKey,
        'X-FluxWa-Delivery-Id': deliveryId,
        'X-FluxWa-Retry-Count': '0',
      };
  ...
  private async enqueueWithFallback(
    ...
    try {
      ...
      if (webhook.secret) {
        headers['X-FluxWa-Signature'] = signature;
      }
      ...
      // same X-FluxWa-Idempotency-Key / X-FluxWa-Delivery-Id, so a conformant receiver dedupes.
      try {
  ...
  private async deliverWebhook(
    ...
    headers['X-FluxWa-Retry-Count'] = String(attempt - 1);

    // Add signature if secret is configured and not already present
    if (webhook.secret && !headers['X-FluxWa-Signature']) {
      headers['X-FluxWa-Signature'] = this.generateSignature(body, webhook.secret);
    }
   ...
   * X-FluxWa-* header) so a webhook config cannot forge the signature/event/idempotency
  ...
  sanitizeCustomHeaders(custom: Record<string, string> | null | undefined): Record<string, string> {
    ...
    for (const [key, value] of Object.entries(custom ?? {})) {
      if (!/^(content-type|x-01flux-wa-)/i.test(key)) {
        safe[key] = value;
      }
    }
    return safe;
