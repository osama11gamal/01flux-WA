...
export class WebhookService implements OnModuleInit, OnModuleDestroy {
  ...
  async test(sessionId: string, webhookId: string): Promise<{ success: boolean; statusCode?: number; error?: string }> {
    ...
    const testPayload: WebhookPayload = {
      ...
      data: {
        message: 'This is a test webhook from 01flux WA',
    ...
    const headers: Record<string, string> = {
      ...
      'X-FluxWa-Event': 'test',
      'X-FluxWa-Idempotency-Key': testPayload.idempotencyKey,
      'X-FluxWa-Delivery-Id': testPayload.deliveryId,
      'X-FluxWa-Retry-Count': '0',
    };

    if (webhook.secret) {
      headers['X-FluxWa-Signature'] = this.delivery.generateSignature(body, webhook.secret);
    }
