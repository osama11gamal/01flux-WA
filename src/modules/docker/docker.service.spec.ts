...
describe('DockerService.getRunningBuiltinServices', () => {
  const container = (name: string, service: string, state: string) => ({
    ...
    state,
    status: state,
    labels: { 'com.01flux-wa.service': service, 'com.01flux-wa.builtin': 'true' },
  });
  ...
  it('reports a service built-in only when its labeled container is actually running', async () => {
    ...
    jest
      ...
      .mockResolvedValue([
        container('01flux-wa-postgres', 'database', 'running'),
        container('01flux-wa-redis', 'cache', 'exited'),
      ]);
...
describe('DockerService.getContainerByService exact-name fallback', () => {
  ...
  // 01flux WA-managed container name, never a substring (a substring — and especially the empty string —
  ...
  it('does not resolve any container for an empty service name', async () => {
    const { service, getContainer } = withFakeDocker([{ Id: 'abc', Names: ['/01flux-wa-postgres'] }]);
  ...
  it('does not resolve a container by substring of its name', async () => {
    const { service, getContainer } = withFakeDocker([{ Id: 'abc', Names: ['/01flux-wa-postgres-primary'] }]);
    // 'postgres' is a substring of '01flux-wa-postgres-primary' but not the exact managed name.
    expect(await service.getContainerByService('postgres')).toBeNull();
    expect(getContainer).not.toHaveBeenCalled();
  });

  it('resolves the exact 01flux-wa-<service> container', async () => {
    const { service, getContainer } = withFakeDocker([
      { Id: 'p', Names: ['/01flux-wa-postgres'] },
      { Id: 'r', Names: ['/01flux-wa-redis'] },
    ]);
...
describe('DockerService.onModuleInit', () => {
  ...
  it('bootstraps the env-configured built-in profiles when the daemon is reachable', async () => {
    ...
    expect(docker.createContainer).toHaveBeenCalledWith(expect.objectContaining({ name: '01flux-wa-redis' }));
  });
...
describe('DockerService.listContainers', () => {
  ...
  it('maps only 01flux WA containers (label or /01flux-wa- name) to ContainerInfo', async () => {
    const service = makeService(
      jest.fn().mockResolvedValue([
        {
          Id: 'aabbccddeeff00112233',
          Names: ['/01flux-wa-postgres'],
          State: 'running',
          Status: 'Up 2 hours',
          Labels: { 'com.01flux-wa.service': 'database', 'com.01flux-wa.builtin': 'true' },
        },
        { Id: '11223344556677889900', Names: ['/01flux-wa-redis'], State: 'exited', Status: 'Exited (0) yesterday' },
        // Label-only match with sparse fields: falls back to 'unknown' placeholders.
        { Id: 'ffee0011223344556677', Labels: { 'com.01flux-wa.service': 'cache' } },
    ...
    expect(await service.listContainers()).toEqual([
      {
        id: 'aabbccddeeff',
        name: '01flux-wa-postgres',
        state: 'running',
        status: 'Up 2 hours',
        labels: { 'com.01flux-wa.service': 'database', 'com.01flux-wa.builtin': 'true' },
      },
      { id: '112233445566', name: '01flux-wa-redis', state: 'exited', status: 'Exited (0) yesterday', labels: {} },
      {
        id: 'ffee00112233',
        name: 'unknown',
        state: 'unknown',
        status: 'unknown',
        labels: { 'com.01flux-wa.service': 'cache' },
      },
    ]);
  });
...
describe('DockerService.getContainerByService label match and guard rails', () => {
  ...
  it('resolves the container by its com.01flux-wa.service label', async () => {
    const service = new DockerService();
    const listContainers = jest.fn().mockResolvedValue([{ Id: 'label-hit-1', Names: ['/01flux-wa-postgres'] }]);
    const getContainer = jest.fn((id: string) => ({ id }));
    Object.assign(service as unknown as Record<string, unknown>, {
      docker: { listContainers, getContainer },
      isAvailable: true,
    ...
    expect(listContainers).toHaveBeenCalledWith({
      all: true,
      filters: { label: ['com.01flux-wa.service=database'] },
    });
...
describe('DockerService.createService', () => {
  ...
  it('pulls, creates the volume and container, and starts it (a pre-existing volume is fine)', async () => {
    ...
    expect(docker.createVolume).toHaveBeenCalledWith({ Name: 'flux_wa_redis-data' });
    expect(docker.createContainer).toHaveBeenCalledWith(
      expect.objectContaining({ name: '01flux-wa-redis', Image: 'redis:7-alpine' }),
    );
