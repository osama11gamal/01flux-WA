...
/**
 * The only Docker profiles 01flux WA manages (and may start/stop). Used to bound teardown so a
...
export class DockerService implements OnModuleInit {
  ...
  /**
   * List all 01flux WA-related containers
   */
  async listContainers(): Promise<ContainerInfo[]> {
    ...
    try {
      ...
      return containers
        .filter(c => {
          // Filter by 01flux WA labels or name prefix
          const labels = c.Labels || {};
          return labels['com.01flux-wa.service'] || c.Names?.some(n => n.startsWith('/01flux-wa-'));
        })
        .map(c => ({
  ...
  /**
   * Which bundled (01flux WA-managed) service containers are currently RUNNING, keyed by the
   * `com.01flux-wa.service` label (`database` | `cache` | `storage`). Lets the dashboard show the real
  ...
  async getRunningBuiltinServices(): Promise<{ database: boolean; cache: boolean; storage: boolean }> {
    ...
    const isRunning = (svc: string): boolean =>
      containers.some(
        c =>
          c.labels['com.01flux-wa.service'] === svc && c.labels['com.01flux-wa.builtin'] === 'true' && c.state === 'running',
      );
  ...
  async getContainerByService(service: string): Promise<Docker.Container | null> {
    ...
    try {
      const containers = await this.docker.listContainers({
        all: true,
        filters: {
          label: [`com.01flux-wa.service=${service}`],
        },
      });
      ...
      // string, would resolve an arbitrary container). 01flux WA-managed containers are `01flux-wa-<service>`.
      const target = `01flux-wa-${service}`;
   ...
   *    (01flux-wa/01flux-wa, minioadmin/minioadmin) that infra-config.controller writes to data/.env.generated
   ...
   *    only 01flux-wa-api carries mem_limit/pids_limit (in compose).
   */
  private getContainerSpec(profile: string): {
    ...
    const specs: Record<string, ReturnType<typeof this.getContainerSpec>> = {
      redis: {
        ...
        volumes: [{ name: 'flux_wa_redis-data', path: '/data' }],
        healthcheck: {
        ...
        labels: {
          'com.01flux-wa.service': 'cache',
          'com.01flux-wa.builtin': 'true',
        },
      ...
      postgres: {
        ...
        env: ['POSTGRES_USER=01flux-wa', 'POSTGRES_PASSWORD=01flux-wa', 'POSTGRES_DB=01flux-wa'],
        volumes: [{ name: 'flux_wa_postgres-data', path: '/var/lib/postgresql/data' }],
        healthcheck: {
          test: ['CMD-SHELL', 'pg_isready -U 01flux-wa'],
          interval: 5000000000,
          timeout: 3000000000,
          retries: 5,
        },
        labels: {
          'com.01flux-wa.service': 'database',
          'com.01flux-wa.builtin': 'true',
        },
        securityOpt: ['no-new-privileges:true'],
      },
      minio: {
        ...
        volumes: [{ name: 'flux_wa_minio-data', path: '/data' }],
        ports: [
