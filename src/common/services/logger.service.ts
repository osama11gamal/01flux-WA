...
export class LoggerService implements NestLoggerService {
  ...
  // (so it lines up with the framework's own output), but tagged honestly as [01flux WA]:
  //   [01flux WA] 1896  - 25/06/2026, 4:59:22 PM     LOG [AuthService] API Docs: … sessionId=abc
  private formatPretty(level: LogLevel, entry: Record<string, unknown>): string {
    ...
    let line =
      `${paint(levelColor, `[01flux WA] ${process.pid}  -`)} ${timestamp} ` +
