...
describe('docs/10 metric table matches MetricsService', () => {
   ...
   * Deliberately NOT restricted to the `FLUX_WA_` prefix: two series are conventionally unprefixed so
  ...
  /** The table rows: `| \`flux_wa_x\` | type | labels | meaning |`. */
  ...
  it('both readers actually read something', () => {
    ...
    expect(emitted().get('flux_wa_up')).toBe('gauge');
    expect(documented().size).toBeGreaterThanOrEqual(10);
    expect(documented().get('flux_wa_up')).toBe('gauge');
  });
