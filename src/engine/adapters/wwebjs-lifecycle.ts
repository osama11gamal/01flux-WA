 ...
 * often a persistent browser profile left stale by an 01flux WA upgrade that changed the Chromium/Chrome
...
export class WwebjsLifecycle {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      // output later (after a hard kill of the 01flux WA process orphaned them).
      puppeteerArgs.push(`--01flux-wa-session=${this.host.config.sessionId}`);
      ...
      if (isExecutionContextDestroyedError(reason)) {
        ...
        this.host.logger.warn(
          `"${reason}" during initialize. If this followed an 01flux WA upgrade that changed the ` +
  ...
  private async runInitAttempt(
    ...
    // Kill any Chromium that survived a hard kill of a previous 01flux WA process lifetime (its
