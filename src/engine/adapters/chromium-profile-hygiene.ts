...
/**
 * SIGKILL any Chromium orphaned by a previous lifetime of this process. When 01flux WA dies hard
 ...
 * `--01flux-wa-session=<id>` marker arg appended to the puppeteer args at launch (Chromium ignores
...
export async function killOrphanedChromiumProcesses(sessionId: string, logger: HygieneLogger): Promise<void> {
  ...
  try {
    ...
    const marker = `--01flux-wa-session=${sessionId}`;
    ...
    for (const line of psOutput.split('\n')) {
      ...
      // (e.g. a `grep --01flux-wa-session=…` probing the process table).
