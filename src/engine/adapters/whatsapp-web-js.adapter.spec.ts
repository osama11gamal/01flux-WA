...
describe('WhatsAppWebJsAdapter ready reconciliation (#251/#273)', () => {
  ...
  // could not tell an 01flux WA self-heal apart from a WhatsApp-side logout or an untouched profile.
...
describe('WhatsAppWebJsAdapter orphaned Chromium sweep (pre-launch)', () => {
  ...
  it('appends the --01flux-wa-session marker to the puppeteer args handed to the Client', async () => {
    const adapter = newAdapter();

    await adapter.initialize({});

    const client = (adapter as unknown as { client: { options: { puppeteer?: { args?: string[] } } } }).client;
    expect(client.options.puppeteer?.args).toContain(`--01flux-wa-session=${SESSION_ID}`);
  });
  ...
  it('does not mutate the caller-owned puppeteer args array shared across sessions', async () => {
    ...
    expect(argsB).toContain('--01flux-wa-session=sess-b');
    ...
    expect(argsB).not.toContain('--01flux-wa-session=sess-a');
  });
  ...
  it('SIGKILLs a Chromium process carrying this session marker and logs the sweep', async () => {
    mockPsResult({
      stdout: psTable([
        [
          1501,
          `/Applications/Google Chrome.app/Contents/MacOS/Google Chrome --headless --01flux-wa-session=${SESSION_ID}`,
        ],
  ...
  it('does NOT kill a non-browser process that merely carries the marker string', async () => {
    mockPsResult({
      stdout: psTable([
        [1601, `/bin/grep --01flux-wa-session=${SESSION_ID}`],
        [1602, `/usr/bin/node scan-sessions.js --01flux-wa-session=${SESSION_ID}`],
      ]),
    });
  ...
  it('does NOT kill a Chromium process belonging to a different session', async () => {
    mockPsResult({
      stdout: psTable([[1701, '/usr/lib/chromium/chromium --headless --no-sandbox --01flux-wa-session=session-lain']]),
    });
  ...
  it('does NOT kill a live sibling whose marker merely SHARES A PREFIX with ours (sess vs sess-2)', async () => {
    // `--01flux-wa-session=sess-orphan` is a substring of `--01flux-wa-session=sess-orphan-2`: a substring
    // match would SIGKILL the sibling's live browser; the token-exact match must spare it.
    mockPsResult({
      stdout: psTable([
        [1801, `/usr/lib/chromium/chromium --headless --no-sandbox --01flux-wa-session=${SESSION_ID}-2`],
        [1802, `/usr/lib/chromium/chromium --headless --no-sandbox --01flux-wa-session=${SESSION_ID}extra`],
      ]),
    });
  ...
  it('kills the orphan when the marker is the LAST token on the command line', async () => {
    mockPsResult({
      stdout: psTable([[1803, `/usr/lib/chromium/chromium --headless --no-sandbox --01flux-wa-session=${SESSION_ID}`]]),
    });
  ...
  it('runs the orphan sweep before the Singleton cleanup and client.initialize()', async () => {
    ...
    execFileSpy.mockImplementation((...args: unknown[]) => {
      ...
      (args[args.length - 1] as ExecFileCallback)(
        null,
        psTable([[1801, `/usr/bin/chromium --headless --01flux-wa-session=${SESSION_ID}`]]),
        '',
      );
    });
