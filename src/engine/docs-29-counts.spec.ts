...
describe('docs/29 counts match the capability matrix', () => {
  ...
  it('states the install-time patch counts the scripts directory actually contains', () => {
    ...
    const spelled = doc.match(/01flux-wa ships (\w+) exact, self-disabling source transforms/);
    ...
    const claims: { label: string; re: RegExp; want: number }[] = [
      ...
      { label: 'mermaid wwjs node', re: /whatsapp-web\.js [\d.]+<br\/>\+ (\d+) 01flux WA patch/, want: expected.wwjs },
      { label: 'mermaid baileys node', re: /baileys [\w.-]+<br\/>\+ (\d+) 01flux WA patch/, want: expected.baileys },
