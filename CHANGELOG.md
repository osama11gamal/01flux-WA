...
- The upgrade runbook and the migration guide state the `docker-compose.dev.yml` caveat before the first
  `docker compose` command instead of after it, and name the `01flux-wa` service substitution it needs.
...
- The `flux_wa_sessions_restricted` gauge now follows restrictions that lapse on their own instead of reporting the pre-expiry count indefinitely.
...
- `GET /api/metrics` no longer `500`s when the data database is unreachable; database-derived series are omitted rather than zeroed, and a new `flux_wa_stats_available` gauge says which happened. The metrics reference now lists every emitted series, gated against the renderer.
...
- The published image's drop to the `01flux-wa` user is now verified in CI — the smoke test previously ran in no workflow.
...
- The chart's optional ServiceMonitor selects on a new `01flux-wa.io/scrape-target` label, scraping one target per pod instead of two — check anything keyed on the `service` label.
...
- `rmyndharis-01flux-wa` 0.2.0 on PyPI, the first release through the trusted-publishing workflow.
- `rmyndharis/01flux-wa` 0.2.0 on Packagist, the first versioned PHP release since June; Composer users on a stable constraint were pinned to 0.1.0.
...
- The JavaScript SDK publishes to npm from CI via Trusted Publishing (OIDC) on a `js-sdk-v*` tag — no npm token exists anywhere, and every release carries build provenance. First release: `@rmyndharis/01flux-wa@0.2.0`.
...
- Helm chart for Kubernetes deployments under `charts/01flux-wa/`. Closes #695.
...
- `scripts/01flux-wa.sh`, an orchestration helper superseded by the in-process Docker orchestration on `/api/infra`.
...
- Orphan-Chromium sweep now matches the `--01flux-wa-session=<id>` marker token-exactly, so restarting `sales` no longer kills sibling `sales2`. (#923)
...
- Completed `.dockerignore` (`.git/`, `dashboard/node_modules`, `*.sqlite`, agent workspaces, etc.) with a `check-dockerignore.mjs` CI gate, and extracted `postinstall` to `scripts/postinstall.js` with failure propagation. **Breaking (behavior):** `docker pull 01flux-wa:latest` no longer moves on a main merge; `npm install` fails on a broken dashboard/patch. (#943)
...
- Release images are now dual-published bit-identical to `docker.io/rmyndharis/01flux-wa` alongside GHCR, with provenance and SBOM attestations verified pullable before release.
...
- The SQLite driver is now the actively maintained `better-sqlite3` (current SQLite 3.53.x with FTS5 fixes); existing database files open as-is and all migrations apply unchanged. ([#848](https://github.com/rmyndharis/01flux-wa/issues/848))
...
- Enabled plugins stay enabled across a gateway restart; enable state is tracked separately from running state, and enabled plugins are restarted after boot. A plugin that fails to start is logged and left disabled ([#856](https://github.com/rmyndharis/01flux-wa/issues/856)).
...
- Docs: the README feature table points to the first-party Integration Fabric plugins in the [01flux WA-plugins](https://github.com/rmyndharis/01flux-wa-plugins) repo, and `docs/23-community-integrations.md` clarifies its community-only scope.
...
- The README gains a **"Before you connect a number"** section: 01flux WA is unofficial, a per-engine ban-risk vs. resource-cost table, six safe-sending guardrails, the known cold-contact first-send silent drop (#830), and a pointer to the official Cloud API. Responds to discussions #87, #154, #436, #687, #694.
...
- The linked-device name is now brandable via the optional `BAILEYS_BROWSER_NAME` env var (default `01flux WA`). Thanks @clicsoluciones. (#822)
...
- Reconnect-loop observability: an `flux_wa_session_reconnect_attempts_total` counter, plus a `session.reconnect_loop` webhook, warning log, and `flux_wa_session_reconnect_loop_alerts_total` tick on every fifth consecutive attempt; the streak re-arms after a stable connection.
- The whatsapp-web.js engine sweeps orphaned Chromium processes carrying an `--01flux-wa-session=<id>` marker before each (re)launch.
...
- Prometheus counter `flux_wa_webhook_delivery_failures_total` on `/api/metrics`, incremented once per delivery that exhausts its retries.
...
- Bundled Compose no longer kills Chromium mid-spawn under multi-session whatsapp-web.js load; `pids_limit` default raised 512 → 2048, exposed as `FLUX_WA_PIDS_LIMIT` (#636).
...
- Official Java SDK (`com.rmyndharis:01flux-wa`): a synchronous Java 17 client covering all 12 REST resources plus API-key validation, published to Maven Central as `com.rmyndharis:01flux-wa:0.1.1`. (#602)
...
- Send true WhatsApp voice notes (PTT): `send-audio`, bulk send, and the `MessageSendAudio` tool accept an optional `ptt`; the server defaults the mimetype to `audio/ogg; codecs=opus` and stores `type: "voice"`. (01flux-wa-n8n #13)
...
- Docker container teardown on `POST /infra/restart` is restricted to the managed allowlist (`postgres`/`redis`/`minio`) with exact `01flux-wa-<service>` matching. (#534)
...
- Client SDKs: official hand-written libraries for JavaScript/TypeScript (`@rmyndharis/01flux-wa`), Python (`rmyndharis-01flux-wa`), and PHP (`rmyndharis/01flux-wa`), each with the same fluent resource surface, a typed error hierarchy, and server-mirroring types; published at `0.1.0`. (#463)
...
- `dashboard/nginx.conf` now targets `01flux-wa-api` for `/api/` and `/socket.io/`. Thanks @Abhishekrajpurohit (#259).
...
- Headless Chromium now starts as the non-root `01flux-wa` user in the Docker image. (closes #242)
...
- Datastore secrets are now required — no `01flux-wa`/`minioadmin` default; production refuses to boot with placeholders.
- Bull Board `?apiKey=` removed — authenticate via `X-API-Key`/`Authorization: Bearer`.
- New env knobs: `SSRF_ALLOWED_HOSTS`, `MEDIA_DOWNLOAD_MAX_BYTES`, `MEDIA_DOWNLOAD_TIMEOUT_MS`, `MAIN_DATABASE_SYNCHRONIZE`, `SHUTDOWN_DELAY_MS`, `FLUX_WA_MEM_LIMIT`, `METRICS_TOKEN`.
...
- Auth: first-boot admin key is a random `flx_k1_` key in all environments; fixed `dev-admin-key` seeded only when `ALLOW_DEV_API_KEY=true`. (#221)
...
- Docker: the container reaches the Docker API via a least-privilege `docker-socket-proxy` over TCP; Node runs as non-root `01flux-wa` via a `gosu` entrypoint (`dumb-init` PID 1). Thanks @A831ARD0 (#227, #228; supersedes #129).
...
First stable release of the 01flux WA WhatsApp API Gateway.
