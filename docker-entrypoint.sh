...
# 01flux-wa user via gosu so the Node process never holds root privileges.
set -e
...
# Chromium resolves its home from the passwd entry (no /home/01flux-wa exists), so it hard-crashes at
# launch unless its config/cache dirs exist and are writable. XDG_CONFIG_HOME/XDG_CACHE_HOME (set in
# the image) point here; create them owned by 01flux-wa. On a read_only rootfs these live on tmpfs /tmp,
...
chown fluxwa:fluxwa "${XDG_CONFIG_HOME:-/tmp/.config}" "${XDG_CACHE_HOME:-/tmp/.cache}"
