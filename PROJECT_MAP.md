...
27. **Railway Metal builder vs BuildKit cache mounts**: Metal rejects id-less mounts AND the documented \id=s/<service-id>-<path>\ form ('missing the cacheKey prefix' - unresolved upstream as of 2026-08). Dockerfile uses plain npm ci + layer caching instead; re-adding mounts only if Railway fixes Metal.
