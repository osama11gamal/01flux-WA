...
Official PHP SDK for the [01flux WA](https://github.com/rmyndharis/01flux-wa) WhatsApp API Gateway.
...
Requires PHP 8.1+ and Guzzle 7. The namespace is `01flux WA\`.
...
A non-2xx response throws a typed `01flux WA\Exceptions\FluxWaApiException` subclass —
`FluxWaAuthException` (401), `FluxWaForbiddenException` (403), `FluxWaNotFoundException` (404),
`FluxWaConflictException` (409), `FluxWaRateLimitException` (429),
`FluxWaNotImplementedException` (501), `FluxWaServiceUnavailableException` (503 — the only
retryable one) — each exposing `getStatus()` and the parsed `getBody()`.
A timeout throws `FluxWaTimeoutException`.

```php
use 01flux WA\Exceptions\FluxWaNotFoundException;

try {
    $client->sessions->get('missing');
} catch (FluxWaNotFoundException $e) {
...
[`rmyndharis/01flux-wa-php`](https://github.com/rmyndharis/01flux-wa-php), not from the
