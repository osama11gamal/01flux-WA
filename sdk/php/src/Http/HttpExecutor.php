...
/**
 * Injectable HTTP transport for the 01flux WA SDK.
 *
...
class HttpExecutor
{
     ...
     *
     * @throws FluxWaApiException  On any non-2xx response (typed subclass).
     * @throws FluxWaTimeoutException On timeout.
     */
    ...
    private function send(string $method, string $path, array $query, $body): ResponseInterface
    {
        ...
        try {
            ...
            if ($isTimeout) {
                throw new FluxWaTimeoutException($this->timeout);
            }
            throw $e;
        }
    ...
    private function buildApiException(ResponseInterface $response, string $method, string $path): FluxWaApiException
    {
        $status = $response->getStatusCode();
        $text = (string) $response->getBody();
        $data = null;
        ...
        $message = "01flux WA API {$status} {$reason} — {$method} {$path}: {$messageText}";

        return FluxWaApiException::classify($status, $message, $data, $envelope['error'] ?? null);
    }
}
