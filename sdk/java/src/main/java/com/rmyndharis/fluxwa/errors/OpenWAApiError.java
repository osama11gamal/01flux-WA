 ...
 * subclass.
 */
public class FluxWaApiError extends FluxWaError {
    ...
    public FluxWaApiError(String message, int status, Object body, String errorKind) {
        super(message);
        this.status = status;
        this.body = body;
     ...
     * surfaces as a generic {@link FluxWaApiError} whose message states the
     * redirect was not followed (the API key is never re-sent to a redirect
     * target).
     */
    public static FluxWaApiError fromResponse(int status, String statusText, String rawBody, String context) {
        ...
        String message = "01flux WA API " + status + reason + " — " + context + tail;
    ...
    public static FluxWaApiError classify(int status, String message, Object body, String errorKind) {
        return switch (status) {
            case 401 -> new FluxWaAuthError(message, status, body, errorKind);
            case 403 -> new FluxWaForbiddenError(message, status, body, errorKind);
            case 404 -> new FluxWaNotFoundError(message, status, body, errorKind);
            case 409 -> new FluxWaConflictError(message, status, body, errorKind);
            case 429 -> new FluxWaRateLimitError(message, status, body, errorKind);
            case 501 -> new FluxWaNotImplementedError(message, status, body, errorKind);
            case 503 -> new FluxWaServiceUnavailableError(message, status, body, errorKind);
            default -> new FluxWaApiError(message, status, body, errorKind);
        };
    }
}
