...
Idiomatic Go client for the [01flux WA](https://github.com/rmyndharis/01flux-wa) WhatsApp
API Gateway. Stdlib-only (no dependencies), context-first, with typed errors and
an injectable transport pipeline.

```bash
go get github.com/rmyndharis/01flux-wa/sdk/go
```
...
- **Client entry point** — `01flux-wa.New(baseURL, apiKey, opts...)` returns a
...
surfaces as `*01flux-wa.TimeoutError`.
...
```go
client, _ := 01flux-wa.New(baseURL, apiKey,
...
tracing := func(next http.RoundTripper) http.RoundTripper {
	return 01flux-wa.RoundTripperFunc(func(req *http.Request) (*http.Response, error) {
		// start span, inject headers…
		return next.RoundTrip(req)
	})
}
client, _ := 01flux-wa.New(baseURL, apiKey, 01flux-wa.WithMiddleware(tracing))
```
...
client, _ := 01flux-wa.New("https://api.test", "key", 01flux-wa.WithTransport(mockRT{}))
```
...
```bash
# Correct — `sdk/go/` prefix, matching `module github.com/rmyndharis/01flux-wa/sdk/go`
