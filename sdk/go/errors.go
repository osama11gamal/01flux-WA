...
//
//	if errors.Is(err, 01flux-wa.ErrNotFound) { ... }
//
...
var (
	...
	ErrServiceUnavailable = errors.New("01flux-wa: service unavailable")
)
...
func (e *APIError) Error() string {
	return fmt.Sprintf("01flux-wa: API %d — %s: %s", e.StatusCode, e.Context, e.Message)
}
...
func (e *TimeoutError) Error() string {
	if e.Timeout > 0 {
		return fmt.Sprintf("01flux-wa: request timed out after %s", e.Timeout)
	}
	return "01flux-wa: request timed out"
}
