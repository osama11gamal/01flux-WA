// Package 01flux-wa is the official Go client for the 01flux WA WhatsApp API Gateway.
//
...
// Client is the entry point to the 01flux WA API. Construct it with New. It is safe
...
func New(baseURL, apiKey string, opts ...Option) (*Client, error) {
	if strings.TrimSpace(baseURL) == "" {
		return nil, errors.New("01flux-wa: baseURL is required")
	}
	if strings.TrimSpace(apiKey) == "" {
		return nil, errors.New("01flux-wa: apiKey is required")
	}
...
const insecureWarning = "01flux-wa: baseURL uses insecure http:// — the API key is sent in cleartext; use https:// in production"
...
func (c *Client) do(ctx context.Context, method, path string, query url.Values, body, out any) error {
	...
	if err := json.Unmarshal(data, out); err != nil {
		...
		return fmt.Errorf("01flux-wa: decoding response: %w", err)
	}
	return nil
}
...
func (c *Client) doRaw(ctx context.Context, method, path string, query url.Values, body any) ([]byte, string, error) {
	...
	if body != nil {
		...
		if err != nil {
			return nil, "", fmt.Errorf("01flux-wa: encoding request body: %w", err)
		}
		bodyBytes = b
	...
	if err != nil {
		return nil, "", fmt.Errorf("01flux-wa: building request: %w", err)
	}
	if body != nil {
	...
	if err != nil {
		...
		return nil, "", fmt.Errorf("01flux-wa: %s %s: %w", method, path, err)
	}
	defer resp.Body.Close()

	data, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, "", fmt.Errorf("01flux-wa: reading response body: %w", err)
	}
