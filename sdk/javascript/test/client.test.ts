...
function client(transport: MockTransport): FluxWaClient {
  return new FluxWaClient({
...
describe('FluxWaClient', () => {
  it('requires baseUrl and apiKey', () => {
    expect(() => new FluxWaClient({ baseUrl: '', apiKey: 'x' })).toThrow();
    expect(() => new FluxWaClient({ baseUrl: 'http://x', apiKey: '' })).toThrow();
  });
  ...
  it('sends the API key as X-API-Key and JSON content type', async () => {
    ...
    expect(t.lastCall!.headers['x-api-key']).toBe('flx_k1_test');
    expect(t.lastCall!.headers['content-type']).toBe('application/json');
  });

  it('strips a trailing slash from baseUrl', async () => {
    const t = new MockTransport().on('GET', '/api/sessions', { body: [] });
    const c = new FluxWaClient({ baseUrl: 'http://localhost:2785/', apiKey: 'k', fetch: t.asFetch() });
  ...
  it('does not auto-follow redirects (passes redirect: manual to fetch)', async () => {
    ...
    const c = new FluxWaClient({ baseUrl: 'http://x', apiKey: 'k', fetch: recordingFetch });
  ...
  it('treats an unfollowed redirect (3xx) as an error', async () => {
    ...
    const c = new FluxWaClient({ baseUrl: 'http://x', apiKey: 'k', fetch: redirectingFetch });
    await expect(c.sessions.list()).rejects.toThrow();
  });

  it('surfaces a real opaque unfollowed redirect (status 0) as a clear FluxWaApiError', async () => {
    ...
    const c = new FluxWaClient({ baseUrl: 'http://x', apiKey: 'k', fetch: opaqueRedirectFetch });
    const err = await c.sessions.list().catch((e: unknown) => e);
    expect(err).toBeInstanceOf(FluxWaApiError);
    expect((err as FluxWaApiError).status).toBe(0);
    expect((err as FluxWaApiError).message).toMatch(/redirect/i);
  });
  ...
  it('maps a 404 to FluxWaNotFoundError with parsed body', async () => {
    const t = new MockTransport().on('GET', '/api/sessions/missing', {
      status: 404,
      body: { statusCode: 404, message: 'Session not found', error: 'Not Found' },
    });
    await expect(client(t).sessions.get('missing')).rejects.toBeInstanceOf(FluxWaNotFoundError);
    await expect(client(t).sessions.get('missing')).rejects.toMatchObject({ status: 404 });
  });

  it('maps a 503 to FluxWaServiceUnavailableError', async () => {
  ...
  it('FluxWaApiError.fromResponse parses the NestJS envelope', async () => {
  ...
  it('parses the envelope a production gateway actually sends, with no `error` key', async () => {
    ...
    expect(err).toBeInstanceOf(FluxWaApiError);
    expect((err as FluxWaApiError).message).toContain('Bad Request');
    expect((err as FluxWaApiError).message).not.toContain('[object Object]');
    expect((err as FluxWaApiError).status).toBe(400);
    // No `error` key was sent, so there is no kind to report — undefined, not a stringified object.
    expect((err as FluxWaApiError).errorKind).toBeUndefined();
  });
  ...
  it('still reads `error` as the kind when the gateway sends one', async () => {
    ...
    expect((err as FluxWaApiError).errorKind).toBe('Bad Request');
    expect((err as FluxWaApiError).message).toContain('name must be a string');
  });

  it('maps each status code to its typed error subclass', async () => {
    const cases: Array<[number, new (...a: never[]) => FluxWaApiError]> = [
      [401, FluxWaAuthError],
  ...
  it('falls back to the generic FluxWaApiError (with .status) for an unmapped status', async () => {
    const t = new MockTransport().on('GET', '/api/sessions', {
      status: 418,
      body: { statusCode: 418, message: 'teapot', error: 'Teapot' },
    });
    await expect(client(t).sessions.list()).rejects.toMatchObject({ status: 418 });
    await expect(client(t).sessions.list()).rejects.toBeInstanceOf(FluxWaApiError);
  });

  it('throws FluxWaTimeoutError when the request aborts', async () => {
    const abortingFetch: FetchLike = async () => {
      const e = new Error('aborted');
      e.name = 'AbortError';
      throw e;
    };
    const c = new FluxWaClient({ baseUrl: 'http://x', apiKey: 'k', fetch: abortingFetch });
    await expect(c.sessions.list()).rejects.toBeInstanceOf(FluxWaTimeoutError);
  });
  ...
  it('keeps the timeout armed while reading a stalled response body', async () => {
    ...
    const c = new FluxWaClient({ baseUrl: 'http://x', apiKey: 'k', timeoutMs: 5, fetch: stalledBodyFetch });
