...
import { classifyApiError, FluxWaApiError, FluxWaTimeoutError } from './errors.js';
...
export interface ClientConfig {
  /** Base URL of the 01flux WA API, e.g. `http://localhost:2785`. */
  baseUrl: string;
...
/**
 * Perform a single request against the 01flux WA API and return the parsed JSON
 * body (or `null` for 204). Throws a typed {@link FluxWaApiError} subclass on
 * non-2xx, or {@link FluxWaTimeoutError} on timeout.
 */
...
async function send<T>(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      const apiError = await FluxWaApiError.fromResponse(res, context);
    ...
    if (err instanceof Error && err.name === 'AbortError') {
      throw new FluxWaTimeoutError(timeoutMs);
    }
    throw err;
  } finally {
    clearTimeout(timer);
...
export function warnIfInsecureHttpUrl(url: string, label = 'baseUrl'): void {
  try {
    ...
    if (parsed.protocol === 'http:' && !LOCALHOST_HOSTS.has(parsed.hostname.toLowerCase())) {
      console.warn(
        `[01flux WA SDK] ${label} uses an insecure http:// URL (host: ${parsed.hostname}). ` +
