/**
 * 01flux WA JavaScript/TypeScript SDK — client core.
 *
 * The {@link FluxWaClient} is the single entry point. It holds configuration
...
export interface FluxWaClientOptions {
...
export class FluxWaClient {
  private readonly config: Required<Omit<ClientConfig, 'fetch'>> & { fetch: FetchLike };

  constructor(options: FluxWaClientOptions) {
    if (!options.baseUrl) throw new Error('FluxWaClient: baseUrl is required');
    if (!options.apiKey) throw new Error('FluxWaClient: apiKey is required');
