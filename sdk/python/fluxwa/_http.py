"""HTTP transport wrapper for the 01flux WA Python SDK.
...
from .errors import FluxWaApiError, FluxWaTimeoutError, classify
...
class HttpExecutor:
    ...
    Constructed once per :class:`FluxWaClient`; the transport is taken from
    ...
    def _send(self, method: HttpMethod, path: str, *, query: Mapping[str, Any] | None, body: Any) -> httpx.Response:
        ...
        try:
            ...
            raise FluxWaTimeoutError(self._timeout) from e
        ...
        if res.status_code >= 300:
            ...
            raise FluxWaApiError.from_response(res.status_code, res.text, context)
        return res
