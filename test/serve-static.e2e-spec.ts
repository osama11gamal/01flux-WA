 ...
 * `~/.01flux-wa`, a CI checkout under `~/.cache`, and a `TMPDIR` inside a dotdir all produce it.
...
function makeBuild(root: string): string {
  ...
  writeFileSync(join(root, 'index.html'), '<!doctype html><title>01flux WA Dashboard</title>');
...
const plainDist = makeBuild(join(mkdtempSync(join(dotFreeBase, '01flux-wa-dash-plain-')), 'dashboard', 'dist'));
const dottedDist = makeBuild(join(mkdtempSync(join(osTmp, '01flux-wa-dash-')), '.dotted', 'dashboard', 'dist'));
...
const RENDER_DISABLED = '/__flux_wa_spa_fallback_owned_by_main_ts__';
...
describe.each([
  ...
  it('serves the dashboard index.html at /', async () => {
    ...
    expect(res.text).toContain('01flux WA Dashboard');
  });
  ...
  it('serves built assets with their own content-type (not the SPA shell)', async () => {
    ...
    expect(res.text).not.toContain('01flux WA Dashboard');
  });
