...
const e2ePluginState = join(tmpdir(), `01flux-wa-e2e-plugins-${e2eRunId}`);
...
for (const [key, name] of [
  ...
  process.env[key] = join(tmpdir(), `01flux-wa-e2e-${name}-${e2eRunId}`);
}
...
const e2eDataDb = join(tmpdir(), `01flux-wa-e2e-${process.pid}.sqlite`);
...
const e2eMainDb = join(tmpdir(), `01flux-wa-e2e-main-${process.pid}.sqlite`);
...
const e2eKeyFile = join(tmpdir(), `01flux-wa-e2e-key-${process.pid}`);
