...
# (`docker exec 01flux-wa npm run cli …`, `npm run export`), and its own bundled dependency tree is
...
RUN if [ "$TARGETARCH" = arm64 ]; then \
        ...
        chown fluxwa:fluxwa /opt/puppeteer && \
...
RUN mkdir -p ./data/sessions ./data/media ./data/plugins && \
    chown fluxwa:fluxwa ./data

# The non-root 01flux-wa user has no home of its own (`useradd -r`, no -m). Chromium resolves the home
# dir from the passwd entry via glib's getpwuid() — it IGNORES $HOME — so it tries to read/write
# /home/01flux-wa, which does not exist. On hardened/read-only hosts that makes the browser HARD-CRASH
...
# bypassing the passwd lookup); docker-entrypoint.sh creates them owned by 01flux-wa. On a read_only
...
# Copy entrypoint: runs as root to fix named-volume ownership, then drops to 01flux-wa via gosu
...
# then drops to the fluxwa user via gosu before starting the node process.
#
# NOTE — no `USER 01flux-wa` directive on purpose (Trivy DS-0002 will flag it, ignore).
# The Node process does NOT run as root: docker-entrypoint.sh:30 is
# `exec gosu fluxwa "$@"` after the chowns on lines 7 and 25. Adding `USER 01flux-wa`
# here would run the entrypoint as 01flux-wa and break the chown-before-drop pattern
