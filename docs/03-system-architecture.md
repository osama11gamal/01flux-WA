...
flowchart TB
    ...
    subgraph 01flux WA["01flux WA Platform"]
...
01flux WA is designed with a **Pluggable Architecture** that allows infrastructure components to be swapped without changing application code. This enables flexible deployments ranging from minimal single-session bots to larger single-node, multi-session installs.

> **Note — single-instance:** the live WhatsApp engine layer is stateful and held in-process
> (an in-memory `Map` in `EngineRegistry`). 01flux WA currently runs as **one API instance per
...
01flux WA does **not** use a dynamic `AdaptersModule` or string DI tokens. `AppModule`
...
flowchart TB
    subgraph Production["Production Environment"]
        API[01flux WA API<br/>single instance]
...
> session registry, node claim, or affinity router in the codebase. 01flux WA runs as one API instance
...
> [!IMPORTANT]
> Engine abstraction is critical to mitigate **R001: WhatsApp Protocol Changes** in Risk Management. 01flux WA ships two production-ready engines selectable via `ENGINE_TYPE`: `whatsapp-web.js` (default, Chromium/Puppeteer-based) and `baileys` (browser-free, WebSocket/Noise protocol). With the abstraction layer, adding further engines requires no changes to application code.
...
01flux WA uses the adapter pattern for infrastructure components that can be swapped per deployment needs. This allows users with limited resources to run 01flux WA without heavyweight external dependencies.

### Adapter Overview

```mermaid
flowchart TB
    subgraph Core["01flux WA Core"]
...
01flux WA supports SQLite for lightweight deployments and PostgreSQL for high-volume production.
...
if (dbType === 'postgres') {
  return {
    ...
    database: configService.get('dataDatabase.name', '01flux-wa'),
...
> **Note:** 01flux WA does not currently apply SQLite-specific concurrency hardening. There is **no**
...
01flux WA provides several deployment profiles for different needs:
...
> 01flux WA runs as a single API instance per session-data volume; there is no cluster-mode flag.
...
01flux WA does not auto-detect a profile at runtime; pick one by available resources and expected load:
