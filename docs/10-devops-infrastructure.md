...
01flux WA is a **single-process** application, so a deployment is exactly one app instance per
...
flowchart TB
    ...
    subgraph Deployment["Deployment (single server)"]
        PROXY[Reverse Proxy]
        PROXY --> APP[01flux WA - one instance]
...
# `USER 01flux-wa` directive and no `chown -R fluxwa /app /opt/puppeteer`: a full /app chown walks
...
# `exec gosu fluxwa node dist/main.js` (no-new-privileges blocks any setuid path back up).
RUN groupadd -r fluxwa && useradd -r -g fluxwa fluxwa
...
services:
  app:
    ...
    environment:
      ...
      - DATABASE_PORT=5432
      - DATABASE_NAME=01flux-wa
      - DATABASE_USERNAME=01flux-wa
      - DATABASE_PASSWORD=01flux-wa
      - REDIS_ENABLED=true
      - REDIS_HOST=redis
      - REDIS_PORT=6379
  ...
  postgres:
    ...
    environment:
      - POSTGRES_USER=01flux-wa
      - POSTGRES_PASSWORD=01flux-wa
      - POSTGRES_DB=01flux-wa
    volumes:
...
services:
  app:
    image: ghcr.io/rmyndharis/01flux-wa:latest
    deploy:
      replicas: 1
      resources:
        limits:
...
The maintained way to deploy on Kubernetes is the Helm chart at `charts/01flux-wa/`:
...
`charts/01flux-wa/README.md` and the inline comments in `charts/01flux-wa/values.yaml`.
...
> **Design sketch, not a supported topology.** 01flux WA is single-process with in-memory engine state,
> so the multi-`01flux WA` fan-out below would corrupt WhatsApp auth across replicas. It is retained only
...
export default () => ({
  ...
  dataDatabase: {
    ...
    name: process.env.DATABASE_NAME || '01flux-wa',
...
These rules use the metric names 01flux WA actually exports (`FLUX_WA_*`). The memory rule below uses a
node-exporter metric — an **external** exporter, not the app — and is kept as a host-level example.

```yaml
# monitoring/alerts.yml
groups:
  - name: 01flux-wa-alerts
    rules:
      # Service Down — flux_wa_up disappears (or the scrape fails)
      - alert: ServiceDown
        expr: up{job="01flux-wa"} == 0 or absent(flux_wa_up)
        for: 1m
        labels:
          severity: critical
        annotations:
          summary: '01flux WA service is down'
          description: 'The 01flux WA application is not responding'

      # Session(s) disconnected
      - alert: SessionDisconnected
        expr: flux_wa_sessions{status="disconnected"} > 0
        for: 2m
        labels:
          severity: warning
        annotations:
      ...
      - alert: FailedMessagesPresent
        expr: flux_wa_messages_failed_total > 0
        for: 5m
        labels:
          severity: warning
        annotations:
      ...
      - alert: HighProcessMemory
        expr: flux_wa_process_resident_memory_bytes > 2e9
        for: 10m
        labels:
          severity: warning
        annotations:
          summary: 'High 01flux WA process memory'
          description: 'RSS is {{ $value | humanize1024 }}B'

      # Host memory pressure — EXTERNAL (node-exporter), not exported by 01flux WA
...
receivers:
  - name: 'slack-notifications'
    slack_configs:
      - channel: '#01flux-wa-alerts'
        send_resolved: true

  - name: 'slack-critical'
    slack_configs:
      - channel: '#01flux-wa-critical'
        send_resolved: true
...
The metrics surface is small, so 01flux WA emits Prometheus text exposition format (v0.0.4) **by hand** —
...
export class MetricsService {
  ...
  async render(): Promise<string> {
    ...
    // `overview` is null on failure, which is what flux_wa_stats_available reports.
    const overview = await this.readOverviewOrNull();
    const mem = process.memoryUsage();
    const lines: string[] = [];
    // ... gauge() helper pushes `# HELP` / `# TYPE` / value lines ...
    gauge('flux_wa_up', '...', 1);
    gauge('flux_wa_process_uptime_seconds', '...', Math.round(process.uptime()));
    gauge('flux_wa_process_resident_memory_bytes', '...', mem.rss);
    gauge('flux_wa_process_heap_used_bytes', '...', mem.heapUsed);
    gauge('flux_wa_stats_available', '...', overview ? 1 : 0);
    if (overview) {
      gauge('flux_wa_sessions_total', '...', overview.sessions.total);
      gauge('flux_wa_sessions_active', '...', overview.sessions.active);
      // flux_wa_sessions{status="..."} — one line per status
      // flux_wa_messages_total{direction="outgoing"|"incoming"}
      // flux_wa_messages_failed_total
    }
    // ... then the process-start counters (webhook delivery failures, session reconnect attempts
    // and loop alerts), flux_wa_sessions_restricted, and the pacing refusals — see the table below
...
| -------------------------------------------- | --------- | ----------------------------------- | -------------------------------------------------------------------------------------------- |
| `flux_wa_up`                                  | gauge     | —                                   | Always `1` when scraped                                                                      |
| `flux_wa_process_uptime_seconds`              | gauge     | —                                   | Process uptime                                                                               |
| `flux_wa_process_resident_memory_bytes`       | gauge     | —                                   | RSS                                                                                          |
| `flux_wa_process_heap_used_bytes`             | gauge     | —                                   | V8 heap used                                                                                 |
| `flux_wa_stats_available`                     | gauge     | —                                   | 1 when the database-derived series below could be read on this scrape, 0 when they could not |
| `flux_wa_sessions_total`                      | gauge     | —                                   | Configured sessions                                                                          |
| `flux_wa_sessions_active`                     | gauge     | —                                   | READY (active) sessions                                                                      |
| `flux_wa_sessions`                            | gauge     | `status`                            | Session count per status                                                                     |
| `flux_wa_messages_total`                      | gauge     | `direction` (`incoming`/`outgoing`) | Current stored messages by direction                                                         |
| `flux_wa_messages_failed_total`               | gauge     | —                                   | Current messages in FAILED state                                                             |
| `flux_wa_webhook_delivery_failures_total`     | counter   | —                                   | Webhook deliveries that terminally failed (all retries exhausted) since process start        |
| `flux_wa_session_reconnect_attempts_total`    | counter   | —                                   | Reconnect attempts scheduled across all sessions since process start                         |
| `flux_wa_session_reconnect_loop_alerts_total` | counter   | —                                   | Reconnect-loop alerts emitted since process start                                            |
| `flux_wa_sessions_restricted`                 | gauge     | —                                   | Sessions whose account WhatsApp is currently restricting                                     |
| `flux_wa_send_pacing_refusals_total`          | counter   | `reason`                            | Sends refused by the pacing governor since process start                                     |
...
without knowing anything about 01flux WA. They come from `src/common/metrics/request-metrics.ts`, which
...
database-derived series (`flux_wa_sessions*`, `flux_wa_messages*`) are **omitted entirely** when the
overview cannot be read — `flux_wa_stats_available` is what tells the two cases apart, so alert on it
rather than reading a missing series as zero. `flux_wa_send_pacing_refusals_total` appears only once
...
> **The database-derived series can be absent.** `flux_wa_sessions_*`, `flux_wa_messages_*` and the per-status
...
> `flux_wa_stats_available` goes to 0. The process, HTTP and webhook series keep being served, so `up` stays 1
> and still means "the process is alive". Alert on `flux_wa_stats_available == 0` for the degradation itself;
> an alert written as `flux_wa_sessions_active == 0` would never fire for it, and one written with `absent()`
> would.

### Grafana Dashboard Definition

```json
// monitoring/grafana/dashboards/01flux-wa.json — panels use the FLUX_WA_* metrics 01flux WA exports
{
  "title": "01flux WA Dashboard",
  "uid": "01flux-wa-main",
  "panels": [
    {
      ...
      "targets": [{ "expr": "flux_wa_sessions_active" }]
    },
    {
      "title": "Stored Outgoing Messages",
      "type": "stat",
      "gridPos": { "x": 6, "y": 0, "w": 6, "h": 4 },
      "targets": [{ "expr": "flux_wa_messages_total{direction=\"outgoing\"}" }]
    },
    {
      "title": "Failed Messages",
      "type": "stat",
      "gridPos": { "x": 12, "y": 0, "w": 6, "h": 4 },
      "targets": [{ "expr": "flux_wa_messages_failed_total" }]
    },
    {
      "title": "Sessions by Status",
      "type": "timeseries",
      "gridPos": { "x": 0, "y": 4, "w": 12, "h": 8 },
      "targets": [{ "expr": "flux_wa_sessions", "legendFormat": "{{status}}" }]
    },
    {
      "title": "Stored Messages by Direction",
      "type": "timeseries",
      "gridPos": { "x": 12, "y": 4, "w": 12, "h": 8 },
      "targets": [{ "expr": "flux_wa_messages_total", "legendFormat": "{{direction}}" }]
    },
    {
      ...
      "targets": [
        { "expr": "flux_wa_process_resident_memory_bytes / 1024 / 1024", "legendFormat": "RSS (MB)" },
        { "expr": "flux_wa_process_heap_used_bytes / 1024 / 1024", "legendFormat": "Heap used (MB)" }
      ]
    },
    {
      "title": "Uptime",
      ...
      "targets": [{ "expr": "flux_wa_process_uptime_seconds" }]
    }
  ]
}
```
...
These are the metrics 01flux WA actually exports at `GET /api/metrics`:

| Category     | Metric                                        | Description                                             | Alert Idea                       |
| ------------ | --------------------------------------------- | ------------------------------------------------------- | -------------------------------- |
| **Liveness** | `flux_wa_up`                                   | Always `1` when scraped (absence/scrape-failure = down) | Target down                      |
| **Sessions** | `flux_wa_sessions_total`                       | Configured sessions                                     | Near your expected session count |
| **Sessions** | `flux_wa_sessions_active`                      | READY (active) sessions                                 | Drops below expected             |
| **Sessions** | `flux_wa_sessions{status="..."}`               | Per-status counts (e.g. `disconnected`, `failed`)       | `disconnected`/`failed` > 0      |
| **Messages** | `flux_wa_messages_total{direction="outgoing"}` | Current stored outgoing messages                        | Unexpected change                |
| **Messages** | `flux_wa_messages_total{direction="incoming"}` | Current stored incoming messages                        | Unexpected change                |
| **Messages** | `flux_wa_messages_failed_total`                | Current messages in FAILED state                        | Above acceptable threshold       |
| **System**   | `flux_wa_process_resident_memory_bytes`        | RSS                                                     | Growth / near limit              |
| **System**   | `flux_wa_process_heap_used_bytes`              | V8 heap used                                            | Growth                           |
| **System**   | `flux_wa_process_uptime_seconds`               | Process uptime                                          | Frequent restarts (resets)       |

> 01flux WA does **not** expose request-rate, latency-histogram, webhook, queue, or Node default
...
01flux WA scales **vertically** — add CPU/RAM to a single instance. The table below is **unbenchmarked
...
**Not currently supported.** 01flux WA is a single-process application with in-memory engine state, so
