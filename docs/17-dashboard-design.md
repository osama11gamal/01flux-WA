...
The dashboard is a web-based management interface for 01flux WA that lets users manage sessions, webhooks, and monitor activity without using the API directly.
...
│  ┌───────────────────────────────────────────────────────────────┐  │
│  │  🔗 Main Webhook                                   ✅ Active   │  │
│  │  https://api.example.com/webhook/01flux-wa                        │  │
...
async function request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  const apiKey = sessionStorage.getItem('flux_wa_api_key');
...
`localStorage` under `flux_wa_theme`:
...
maintainability; the legacy `flux_wa_palette` storage key and the attribute are cleaned up on load.
...
export function useTheme() {
  const [theme, setTheme] = useState<Theme>(/* localStorage 'flux_wa_theme' ?? 'system' */);

  useEffect(() => {
    const root = document.documentElement;
    if (theme === 'system') root.removeAttribute('data-theme');
    else root.setAttribute('data-theme', theme);
    localStorage.setItem('flux_wa_theme', theme);
  }, [theme]);
