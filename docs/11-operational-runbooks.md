...
This document contains Standard Operating Procedures (SOP) for 01flux WA operations, including incident response, maintenance procedures, and troubleshooting guides.
...
# 2. Check container logs. Every `docker compose … 01flux-wa-api` below names the service as the
#    production docker-compose.yml defines it — on docker-compose.dev.yml that service is called
#    `01flux-wa`, so substitute it there. The bare `docker stats`/`docker restart` forms take the
#    container name, which is `01flux-wa-api` under both files.
docker compose logs --tail=100 01flux-wa-api
...
# A. Container crashed
docker compose logs 01flux-wa-api 2>&1 | grep -i "error\|fatal\|crash"

# B. Out of memory
docker compose logs 01flux-wa-api 2>&1 | grep -i "oom\|memory"

# C. Database connection
docker compose logs 01flux-wa-api 2>&1 | grep -i "database\|connection refused"
...
# D. Database issues (built-in PostgreSQL runs as container `01flux-wa-postgres`, both when
#    started via the compose `postgres`/`full` profile and when orchestrated by the app)
docker restart 01flux-wa-postgres
...
docker compose logs 01flux-wa-api 2>&1 | grep -i "{sessionId}" | tail -20
...
```bash
# 1. Check current memory usage (the container is named `01flux-wa-api`)
docker stats --no-stream 01flux-wa-api
free -m
...
curl -H "Authorization: Bearer $METRICS_TOKEN" \
  ...
  | grep -E "flux_wa_process_resident_memory_bytes|flux_wa_process_heap_used_bytes"

# 3. Check for memory leaks
docker compose logs 01flux-wa-api 2>&1 | grep -i "heap\|memory\|gc"
...
docker compose logs 01flux-wa-api 2>&1 | grep -i "webhook" | tail -20

# 3. Identify failure reason:
# A. Endpoint not responding
curl -v https://your-webhook-endpoint.com/01flux-wa

# B. SSL certificate issues
curl -v --insecure https://your-webhook-endpoint.com/01flux-wa

# C. Timeout
curl -v --max-time 30 https://your-webhook-endpoint.com/01flux-wa

# D. Authentication failed
curl -v -H "Authorization: Bearer token" \
  https://your-webhook-endpoint.com/01flux-wa
...
# Verify backup (backup.sh writes $BACKUP_DIR/01flux-wa-backup-<timestamp>.tar.gz,
# BACKUP_DIR defaults to ./backups — it creates no dated subdirectories)
ls -la ./backups/01flux-wa-backup-*.tar.gz
...
#    and the archive is written as $BACKUP_DIR/01flux-wa-backup-<timestamp>.tar.gz)
export BACKUP_DIR="/backups/01flux-wa"
./scripts/backup.sh
...
# to every docker compose command in this runbook, the Rollback block included, and write `01flux-wa`
# wherever a command names the `01flux-wa-api` service (steps 6 and 7, rollback step 2).

# 4. Stop services
docker compose down

# 5. Fetch the new release
# The shipped docker-compose.yml BUILDS 01flux-wa-api from source (`build: context: .`) — there is
...
docker compose build 01flux-wa-api
...
docker compose run --rm 01flux-wa-api npm run migration:run:prod
...
> `image: ghcr.io/rmyndharis/01flux-wa:<tag>` — replace steps 5-6 with editing that tag and running
> `docker compose pull`.
...
#    "$BACKUP_DIR/01flux-wa-backup-<timestamp>.tar.gz". The databases in place still hold the failed
#    upgrade's data, so the restore refuses to touch them without --force
./scripts/restore.sh "$BACKUP_DIR/01flux-wa-backup-<timestamp>.tar.gz" --force
...
#   - plugin-state/    — registry + ctx.storage state under FLUX_WA_DATA_DIR
...
# FLUX_WA_DATA_DIR. A missing source database fails the run (no silent empty backup), the finished
...
# Run from the repo root (database defaults are ./data/...; state dirs follow FLUX_WA_DATA_DIR):
./scripts/backup.sh

# Customize via environment:
FLUX_WA_DATA_DIR=/srv/01flux-wa/data \
  BACKUP_DIR=/backups/01flux-wa \
  DATABASE_TYPE=postgres DATABASE_URL=postgres://user:pass@host:5432/01flux-wa \
  ./scripts/backup.sh
```

> The data directory is a Docker **named volume** (`01flux-wa-data`) in the production
> compose. Run the script where that volume is mounted — e.g. point `FLUX_WA_DATA_DIR`
...
tar -tzf ./backups/01flux-wa-backup-*.tar.gz
```
...
#    the app reads; non-DB state follows FLUX_WA_DATA_DIR. Pass --strict to refuse an archive
...
./scripts/restore.sh ./backups/01flux-wa-backup-<timestamp>.tar.gz
...
du -sh "$(docker inspect --format='{{.LogPath}}' 01flux-wa-api)"
...
#    growing back)
sudo truncate -s 0 "$(docker inspect --format='{{.LogPath}}' 01flux-wa-api)"
