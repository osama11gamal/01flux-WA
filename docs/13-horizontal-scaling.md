...
> (`x-01flux-wa-served-by` names it). The forward happens after API-key auth, carries the
...
This guide explains a _proposed_ design for deploying 01flux WA in a horizontally scaled environment for high availability and increased capacity.
...
flowchart TB
    ...
    subgraph Nodes["01flux WA Nodes"]
        N1[01flux WA Node 1]
        N2[01flux WA Node 2]
        N3[01flux WA Node 3]
    end
...
services:
  ...
  # NOTE (v0.4.0): 01flux WA no longer ships a bundled Traefik container.
  # For TLS / public exposure, bring your own reverse proxy (Traefik, nginx,
  # Caddy, a cloud load balancer, etc.) and point it at 01flux-wa:2785.
...
# Deploy stack
docker stack deploy -c docker-compose.swarm.yml 01flux-wa

# Check status
docker service ls
docker service ps FLUX_WA_01flux WA
```

> **Do not scale the `01flux-wa` service** (`docker service scale FLUX_WA_01flux WA=N`). The `sessions`
...
metadata:
  ...
  namespace: 01flux-wa
spec:
  serviceName: 01flux-wa-headless # must match the headless Service declared in k8s/service.yaml
...
kind: Ingress
metadata:
  name: 01flux-wa-ingress
  namespace: 01flux-wa
  annotations:
    nginx.ingress.kubernetes.io/affinity: 'cookie'
    nginx.ingress.kubernetes.io/session-cookie-name: '01flux-wa-session'
    nginx.ingress.kubernetes.io/session-cookie-max-age: '172800'
spec:
  ingressClassName: nginx
  rules:
    - host: 01flux-wa.example.com
      http:
        paths:
          - path: /
            pathType: Prefix
  ...
  tls:
    - hosts:
        - 01flux-wa.example.com
      secretName: 01flux-wa-tls
```

### Deploy to Kubernetes

The maintained chart is [`charts/01flux-wa`](../charts/01flux-wa), a single-instance StatefulSet that
...
# Check pods
kubectl get pods -n 01flux-wa

# Check logs
kubectl logs -f statefulset/01flux-wa -n 01flux-wa
```

> **Do not raise `replicas` above 1** (`kubectl scale statefulset 01flux-wa --replicas=N`). Each pod
...
http:
  routers:
    01flux-wa:
      rule: 'Host(`01flux-wa.example.com`)'
      service: 01flux-wa
      middlewares:
        - sticky-session
      ...
      loadBalancer:
        sticky:
          cookie:
            ...
            httpOnly: true
        servers:
          - url: 'http://01flux-wa-1:2785'
          - url: 'http://01flux-wa-2:2785'
          - url: 'http://01flux-wa-3:2785'
        healthCheck:
          path: /api/health
          interval: 10s
          timeout: 3s
...
upstream 01flux-wa {
    ...
    server 01flux-wa-1:2785 weight=1 max_fails=3 fail_timeout=30s;
    server 01flux-wa-2:2785 weight=1 max_fails=3 fail_timeout=30s;
    server 01flux-wa-3:2785 weight=1 max_fails=3 fail_timeout=30s;
}

server {
    listen 80;
    server_name 01flux-wa.example.com;

    location / {
        proxy_pass http://01flux-wa;
...
01flux WA exports Prometheus text exposition at `GET /api/metrics` (`FLUX_WA_*` gauges and counters).
...
scrape_configs:
  - job_name: '01flux-wa'
    static_configs:
      # Swarm service name (13.3). On Kubernetes there is no Service called `01flux-wa` — scrape the
      # pod through the headless Service instead, e.g.
      # 01flux-wa-0.01flux-wa-headless.01flux-wa.svc.cluster.local:2785
      - targets: ['01flux-wa:2785']
...
```yaml
# prometheus/01flux-wa-rules.yaml
groups:
  - name: 01flux-wa
    rules:
      - alert: HighMemoryUsage
        expr: container_memory_usage_bytes{container="01flux-wa"} > 1.8e9
        for: 5m
        labels:
          severity: warning
        annotations:
          summary: '01flux WA node high memory usage'

      - alert: NodeDown
        expr: up{job="01flux-wa"} == 0
        for: 1m
        labels:
          severity: critical
        annotations:
          summary: '01flux WA node is down'
```
