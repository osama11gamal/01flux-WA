...
# Check Docker logs (service name in the shipped production compose; it is `01flux-wa` in
# docker-compose.dev.yml)
docker compose logs 01flux-wa-api

# Common fixes
docker system prune -f         # Clean up dangling images/containers
git pull                       # The shipped compose BUILDS 01flux-wa-api from source —
docker compose up -d --build   # `docker compose pull` never updates it
```

> Do **not** reach for `docker compose down --volumes` here. It deletes the `01flux-wa-data` volume,
...
**Cause:** In production 01flux WA sends the CSP `upgrade-insecure-requests` directive, which tells the
...
docker compose exec 01flux-wa-api printenv NODE_ENV CSP_UPGRADE_INSECURE_REQUESTS
```
...
> The alternative is to front 01flux WA with a TLS-terminating reverse proxy (the shipped
...
docker compose logs 01flux-wa-api 2>&1 | grep -i "whatsapp\|puppeteer\|browser"
...
docker compose exec 01flux-wa-api ls -la /app/data/sessions/session-<name>/   # whatsapp-web.js
docker compose exec 01flux-wa-api ls -la /app/data/baileys/<name>/            # baileys
```
...
docker compose exec 01flux-wa-api rm -rf /app/data/sessions/session-<name>   # whatsapp-web.js
docker compose exec 01flux-wa-api rm -rf /app/data/baileys/<name>            # baileys
docker compose restart 01flux-wa-api
```

> The service name above is the one in the shipped production `docker-compose.yml` (`01flux-wa-api`),
> which mounts `/app/data` from a **named volume** — there is no `./data` on the host to inspect;
> reach into the container (`docker compose exec 01flux-wa-api ls /app/data/sessions`) instead. In
> `docker-compose.dev.yml` the app service is called `01flux-wa` and `./data` is bind-mounted, so the
...
**Fix:** 01flux WA reconciles a missed `ready` event when WhatsApp Web is connected, the injected
...
`WWEBJS_WEB_VERSION` unset, `latest`, or `auto` (the default), 01flux WA auto-resolves a settled build
...
`$HOME`**. The non-root `01flux-wa` user has no home dir, so Chromium tries to use `/home/01flux-wa`, which does
...
owned by `01flux-wa`). If you run a custom container, ensure both are set to a writable, existing path:
...
```bash
docker stats 01flux-wa-api   # watch the PIDS column — does it climb toward the limit right before the failure?
```

_Fix:_ raise the ceiling. The bundled `docker-compose.yml` exposes it as `FLUX_WA_PIDS_LIMIT` (default `2048`,
which fits ~8-10 sessions with startup-spike headroom):

```bash
FLUX_WA_PIDS_LIMIT=4096   # in your .env, then docker compose up -d
```
...
# Docker Desktop: check the VM via the app, or nudge FLUX_WA_MEM_LIMIT up and retry
```

_Fix:_ raise the ceiling (`FLUX_WA_MEM_LIMIT=4g` in your `.env`, or Docker Desktop → Settings → Resources →
Memory for the VM).
...
**Quick triage:** run `docker stats 01flux-wa-api`, click **Start**, and watch which resource spikes toward its
...
**Start** after upgrading 01flux WA — no QR is produced — and the session's `lastError` / container log
show:
...
```bash
docker exec 01flux-wa-api rm -rf /app/data/sessions/session-<name>
...
01flux WA returns to a fresh QR because `whatsapp-web.js` deletes LocalAuth credentials on `LOGOUT`.

**Cause:** 01flux WA used to backfill active WhatsApp Status posts immediately in its `ready` callback by
...
> heading ("What's new"). The language WhatsApp Web renders in follows the browser locale, which 01flux WA
...
reads. 01flux WA ships a backport that restores it, applied at install time by
...
docker compose logs 01flux-wa-api 2>&1 | grep -i webhook
...
```bash
# Check memory usage
docker stats 01flux-wa-api --no-stream

# Check process memory (Prometheus text; read flux_wa_process_resident_memory_bytes)
...
services:
  01flux-wa-api:
    # The shipped compose already exposes this as mem_limit: ${FLUX_WA_MEM_LIMIT:-2g}
    mem_limit: 2g
    environment:
...
Pooling and caching are environment variables — 01flux WA has no config file:
...
# Default is: delete (rollback journal) — 01flux WA does not force WAL
...
docker exec 01flux-wa-api ping postgres
docker exec 01flux-wa-api curl http://host.docker.internal:8080
```
...
**Q: Is 01flux WA safe to use?**

> A: 01flux WA uses unofficial WhatsApp Web API. While we implement best practices to avoid detection, there's inherent risk of account restrictions. We recommend:
>
...
> A: Yes, 01flux WA works with both personal and WhatsApp Business accounts. Note that WhatsApp Business API (official Meta API) is different and not supported.
...
> 2. Set URL: `http://01flux-wa:2785/api/sessions/{id}/messages/send-text`
...
**Then set `TRUSTED_PROXIES` in your `.env`.** With it empty, 01flux WA correctly refuses to trust the
...
`127.0.0.1`. 01flux WA logs a one-time warning at the first proxied request when the header is present
but the list is empty.

**Q: How to run behind Traefik / Coolify?**

Traefik forwards WebSocket upgrades automatically, so 01flux WA's single-port Socket.IO channel works with a normal HTTP router. Two things keep a public deployment stable:

**1. Let Traefik reach the container over the Docker network — don't _also_ publish the host port.** This is the most common cause of intermittent `504`s on Coolify/Traefik. If 01flux WA publishes its port to the host (`ports: ["2785:2785"]`) **and** Traefik also routes to it, every request additionally traverses Docker's userland `docker-proxy`. 01flux WA holds a long-lived Socket.IO connection per client (HTTP long-poll → WebSocket upgrade), so those held-open connections accumulate across both hops and gradually exhaust the connection pool to the single upstream — the Dashboard, API, and real-time channel then `504` together "after some time", while `curl http://localhost:2785/api/health/ready` keeps returning `200`. Front it with Traefik on a shared network and **expose** the port internally instead of **publishing** it:
...
services:
  01flux-wa:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.01flux-wa.rule=Host(`api.example.com`)
      - traefik.http.routers.01flux-wa.entrypoints=websecure
      - traefik.http.routers.01flux-wa.tls.certresolver=le
      - traefik.http.services.01flux-wa.loadbalancer.server.port=2785
networks:
  proxy:
...
Remember 01flux WA is **single-port**: the Dashboard, REST API, and Socket.IO all share `:2785` behind one router, so a choked upstream takes all three down at once. A Dashboard stuck on "Connecting…" while `localhost` is healthy is the proxy hop, not the app.

**Q: How to backup sessions automatically?**

```bash
# Add to crontab, for example: 0 */6 * * * cd /path/to/01flux-wa && ./scripts/backup.sh
BACKUP_DIR=/backups/01flux-wa ./scripts/backup.sh
```
...
2. **Check logs** - `docker compose logs 01flux-wa-api --tail=100`
...
- **GitHub Issues**: [github.com/rmyndharis/01flux-wa/issues](https://github.com/rmyndharis/01flux-wa/issues)
- **Discussions**: [github.com/rmyndharis/01flux-wa/discussions](https://github.com/rmyndharis/01flux-wa/discussions)
- **Discord**: [discord.gg/01flux-wa](https://discord.gg/01flux-wa) (if available)
- **Stack Overflow**: Tag with `01flux-wa`
---
