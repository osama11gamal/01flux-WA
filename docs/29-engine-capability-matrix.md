...
7.0.0-rc13), the **whatsapp-web.js library** (1.34.7), and what **01flux WA actually exposes** through
its adapter layer and REST API — including which "supported" cells only work because 01flux WA patches
...
8 install-time patches (29.3). If it exists upstream or in 01flux WA, it has a row here.

## 29.1 How to read this matrix

Statuses used in the tables:

- **✅** — works end-to-end through the 01flux WA adapter.
- **✅🔧ⁿ** — works end-to-end, but **only because 01flux WA patches the installed library** (patch
  `🔧ⁿ`, see 29.3). On a stock, unpatched install of the library this cell would be broken.
- **❌ gap** — _adapter-gap_: the underlying library HAS the capability; only the 01flux WA adapter
...
- **01flux WA REST** column — what a caller of the REST API gets: **✅** on any engine session,
...
- **29.4 — the 01flux WA contract view.** Rows are the 112 `IWhatsAppEngine` methods; use it to see
...
- **29.5 — the full engine inventory.** Rows are **every method the installed libraries expose**,
  each mapped to the 01flux WA interface method that uses it (or marked unexposed). Use it as the
  ...
  have and only 01flux WA lacks.

## 29.2 Adapter architecture

01flux WA never calls a WhatsApp library directly from a controller. Every session owns one engine
instance behind the neutral `IWhatsAppEngine` interface (112 methods +
`EngineEventCallbacks`), and all modules go through it:

```mermaid
flowchart LR
    subgraph 01flux WA["01flux WA"]
        ...
        SVC --> STORE["01flux WA-side stores"]
    end
    WA --> WLIB["whatsapp-web.js 1.34.7<br/>+ 6 01flux WA patches"]
    BA --> BLIB["@whiskeysockets/baileys 7.0.0-rc13<br/>+ 2 01flux WA patches"]
...
- **Inbound events** flow the other way: each adapter normalizes library events into the neutral
  ...
  session module turns into 01flux WA webhook events. Which library events are consumed — and which
...
## 29.3 Install-time patches 01flux WA applies to the libraries

01flux WA ships eight exact, self-disabling source transforms over the installed engines. Each runs at
...
So it is not a house style, and the one method carrying it is the one 01flux WA answers 501 for and
...
## 29.4 Full capability matrix — the 01flux WA contract view (112 methods)

Legend recap: **✅** supported · **✅🔧ⁿ** supported via 01flux WA patch `🔧ⁿ` (29.3) ·
...
Baileys). The **01flux WA REST** column reads from the caller's side: ✅ works whatever engine the
session runs; ⚠️ depends on the session engine; ❌ 501 on both.

### 29.4.1 Session & connection

| Method               | Baileys adapter 🔧⁵ | wwjs adapter 🔧¹ | 01flux WA REST |
| -------------------- | ------------------- | ---------------- | ----------- |
| `initialize`         | ✅                  | ✅🔧⁴            | ✅          |
| `disconnect`         | ✅                  | ✅               | ✅          |
| `logout`             | ✅                  | ✅               | ✅          |
...
## 29.5 Full engine method inventory — every library method, mapped to 01flux WA

This is the backlog view: **all 152 Baileys socket methods and all 81 whatsapp-web.js Client
methods**, each marked with where 01flux WA uses it. The symbol list itself is gated:
...
- **❌ not exposed** — a real library capability with no 01flux WA path. **This is the implementation
  backlog.**
...
| Library method                 | 01flux WA exposure                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
...
with zero 01flux WA surface. Baileys-only; whatsapp-web.js has no community API at all.
...
### 29.5.3 Supported by BOTH libraries — missing only in 01flux WA
...
01flux WA consumes events by normalizing them into `EngineEventCallbacks`; anything else is dropped.
"Consumed" below means referenced by the adapter code.

**Baileys (34):**

| Event                       | 01flux WA                                              |     | Event                            | 01flux WA                          |
| --------------------------- | --------------------------------------------------- | --- | -------------------------------- | ------------------------------- |
