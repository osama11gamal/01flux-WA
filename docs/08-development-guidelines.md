...
01flux WA supports multiple infrastructure configurations. Choose based on your needs:
...
# Optional: seed a known admin key. If omitted, 01flux WA generates a random key and writes data/.api-key.
API_MASTER_KEY=
...
# View Docker logs (service is `01flux-wa-api` in docker-compose.yml, `01flux-wa` in
# docker-compose.dev.yml — there is no service named `app`)
docker compose logs -f 01flux-wa-api
```
...
**Check logs:**
```bash
docker compose logs 01flux-wa-api --tail 100
````
...
docker run --shm-size=2gb 01flux-wa
```
