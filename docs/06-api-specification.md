...
```http
X-API-Key: flx_k1_your-api-key-here
```
...
```http
X-API-Key: flx_k1_your-api-key      # required on every non-public REST route
...
01flux WA seeds an initial admin key on first run (printed to the startup log and written to `data/.api-key`, or `/app/data/.api-key` in Docker). Use it to mint scoped, lower-privilege keys for integrations. Full key creation, listing, rotation, and revocation are documented under the auth resource in **§6.4.9 (API Keys)**.

## 6.2 Response Format

> **01flux WA returns the raw handler payload directly — there is NO `{ success, data, meta }` envelope.** A resource route returns the resource object as-is; a list route returns a **bare JSON array**. Read fields directly (`response.id`, not `response.data.id`).
...
01flux WA uses **two** timestamp representations — be careful which a field is:
...
`docs/19-plugin-architecture.md`), and refusals are counted in the `flux_wa_send_pacing_refusals_total`
...
```json
{ "chatId": "628123456789@c.us", "text": "Hello from 01flux WA!" }
```
...
```json
{ "text": "Hello from 01flux WA!", "recipients": ["6281234567890@c.us"], "backgroundColor": "#25D366", "font": 2 }
```
...
Two fields — `secret` and `headers` — are **write-only**: they are accepted on create/update but are never returned by any webhook route (the response DTO has no `@Expose` for them, so `fromEntity` drops them). `GET /api/infra/export-data` also omits both from its `webhooks` rows, so a backup no longer carries webhook credentials — a restored webhook comes back unsigned (`secret` null, `headers` `{}`) until you set them again. The `secret` is used to compute the `X-FluxWa-Signature: sha256=<hex>` HMAC-SHA256 header on deliveries.
...
| secret     | string                 | no       | `@IsString`, `@MaxLength(255)`                                                                                                                                                                                                                                                                                                                                         | HMAC-SHA256 signing key. **Write-only** — never returned by a webhook route (not returned by `GET /api/infra/export-data` either). Used for `X-FluxWa-Signature`. Defaults to `null`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| headers    | Record<string,string>  | no       | `@IsHeaderMap()` — flat object (not array), ≤50 entries, names match `/^[A-Za-z0-9-]+$/`, values are strings ≤1024 chars with no C0 control/DEL (CR/LF injection guard).                                                                                                                                                                                               | Custom headers added to deliveries. **Write-only** — never returned by a webhook route (not returned by `GET /api/infra/export-data` either). At delivery, `content-type` and `x-01flux-wa-*` names are stripped. Defaults to `{}`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
Same shape as the read DTO **plus** an `apiKey` field carrying the full plaintext key `flx_k1_<64 hex>`. This is the **only** time the plaintext key is returned. `keyPrefix` is the first 12 chars; `usageCount` starts at `0`, `isActive` is `true`. Null `allowedIps`/`allowedSessions`/`expiresAt`/`lastUsedAt` are omitted.
...
```json
{
  ...
  "apiKey": "flx_k1_0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef"
}
```
...
Prometheus exposition scrape of 01flux WA process + session + message metrics; gated by a `METRICS_TOKEN` bearer (disabled when the token is unset).
...
When the data database cannot be read the database-derived series (`flux_wa_sessions*`,
`flux_wa_messages*`) are OMITTED rather than reported as zero — a zero would fire an alert
claiming every session had dropped. `flux_wa_stats_available` is what tells the two cases apart,
so alert on it rather than reading a missing series as zero. `docs/10` lists every series.

```
# HELP flux_wa_up 1 if the 01flux WA process is running
# TYPE flux_wa_up gauge
flux_wa_up 1
# TYPE flux_wa_process_uptime_seconds gauge
flux_wa_process_uptime_seconds 3600
# TYPE flux_wa_process_resident_memory_bytes gauge
flux_wa_process_resident_memory_bytes 187432960
# TYPE flux_wa_process_heap_used_bytes gauge
flux_wa_process_heap_used_bytes 64512000
# TYPE flux_wa_stats_available gauge
flux_wa_stats_available 1
# TYPE flux_wa_sessions_total gauge
flux_wa_sessions_total 3
# TYPE flux_wa_sessions_active gauge
flux_wa_sessions_active 2
# TYPE flux_wa_sessions gauge
flux_wa_sessions{status="ready"} 2
flux_wa_sessions{status="disconnected"} 1
# TYPE flux_wa_messages_total gauge
flux_wa_messages_total{direction="outgoing"} 1280
flux_wa_messages_total{direction="incoming"} 940
# TYPE flux_wa_messages_failed_total gauge
flux_wa_messages_failed_total 4
```
...
| `database.host` / `.port` / `.username` / `.database` | string                   | No                       | `port` is a string                        | External postgres connection (defaults `localhost`/`5432`/`postgres`/`01flux-wa`)                                                          |
...
[
  {
    ...
    "author": "01flux-wa-plugins",
    "download": "https://github.com/01flux-wa-plugins/group-translate/releases/download/v1.2.0/group-translate.zip",
    "installed": true,
...
```json
{ "url": "https://github.com/01flux-wa-plugins/chat-flow/releases/download/v1.0.0/chat-flow.zip" }
```
...
const socket = io('ws://localhost:2785/events', {
  auth: { apiKey: process.env.FLUX_WA_API_KEY },
});
...
`event`, `timestamp` (ISO-8601 dispatch time), `sessionId`, `idempotencyKey`, and `deliveryId` are always present; `data` holds the event-specific payload. The same values are mirrored into request headers (below). The HMAC `signature` is **not** in the body — it travels in the `X-FluxWa-Signature` header.

### Event catalog

These are the events 01flux WA actually emits. A webhook is registered with an `events` list; an event is delivered to a webhook when its `events` array includes the event name or `"*"`.
...
If no `secret` is configured the `X-FluxWa-Signature` header is omitted entirely.
...
| Header                     | Meaning                                                                                                      |
| -------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `X-FluxWa-Event`           | The event name (mirrors `event`)                                                                             |
| `X-FluxWa-Idempotency-Key` | Content-derived key; **stable across retries** of the same occurrence — dedupe on this                       |
| `X-FluxWa-Delivery-Id`     | A fresh `dlv_<uuid>` generated **per delivery** (differs per retry and per webhook) — for tracing, not dedup |
| `X-FluxWa-Retry-Count`     | Retry attempt number (`0` = first attempt)                                                                   |
| `X-FluxWa-Signature`       | HMAC (only when a secret is set)                                                                             |
