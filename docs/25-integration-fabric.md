...
The overriding goal is to preserve the untrusted-worker safety invariants _by construction_. 01flux WA
...
**~90% a faithful clone of seams 01flux WA already ships**:
...
flowchart LR
    ...
    subgraph Core["01flux WA host (core)"]
...
- **Authentication inversion.** A provider webhook cannot carry an 01flux WA API key, so ingress is public to
...
[01flux WA-plugins](https://github.com/rmyndharis/01flux-wa-plugins) catalog, not in this repository. What
