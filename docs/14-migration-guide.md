...
This document provides a comprehensive guide for migrating 01flux WA, including:
...
01flux WA v0.2+ includes built-in migration API endpoints that leverage the **Dual-Database Architecture**:
...
01flux WA v0.2+ supports migrating media files between storage backends:
...
Redis in 01flux WA holds only **ephemeral** state: TTL-based cache entries, BullMQ jobs (see below), and — when `REDIS_ENABLED` — the rate-limit hit counters. Cache data automatically regenerates from the database.
...
> 01flux WA's dependencies (the app itself uses `better-sqlite3`). Install it ad hoc before running:
> `npm install --no-save sqlite3`.
>
> The `SQLITE_PATH` / `DATABASE_URL` variables below are inputs to this standalone script only —
> they are **not** 01flux WA configuration. The application itself reads `DATABASE_TYPE` plus
...
const config: MigrationConfig = {
  ...
  postgresUrl: process.env.DATABASE_URL || 'postgresql://user:pass@localhost:5432/01flux-wa',
...
export DATABASE_NAME=01flux-wa
...
(`01flux-wa-data:/app/data`), not a host bind mount, so the profile is copied through the container
...
```bash
APP_DIR=/srv/01flux-wa            # docker compose project directory on both hosts
...
ssh old-server "cd $APP_DIR && docker compose stop 01flux-wa-api"
ssh new-server "cd $APP_DIR && docker compose stop 01flux-wa-api"
...
ssh old-server "cd $APP_DIR && docker compose cp \
    01flux-wa-api:/app/data/sessions/session-$SESSION_NAME ./session-$SESSION_NAME"
...
ssh new-server "cd $APP_DIR && docker compose cp \
    ./session-$SESSION_NAME 01flux-wa-api:/app/data/sessions/session-$SESSION_NAME"

# 3. Start the target back up.
ssh new-server "cd $APP_DIR && docker compose start 01flux-wa-api"
```
...
#    OLD_DIR/NEW_DIR are each host's 01flux WA working directory; SESSION_DATA_PATH defaults to
#    ./data/sessions and BAILEYS_AUTH_DIR to ./data/baileys, relative to it. The production
#    docker-compose.yml keeps /app/data in the named volume `FLUX_WA_01flux-wa-data` rather than on the
...
01flux WA is pre-1.0 — every release to date is on the `0.x` line. Under the project's SemVer 0.x policy a
...
# Rollback Procedures, and write `01flux-wa` wherever a command names the `01flux-wa-api` service. Do NOT
...
#    Deployments pinned to a published image instead (ghcr.io/rmyndharis/01flux-wa:<version>)
...
# Started with docker-compose.dev.yml (the README Quick Start)? Add `-f docker-compose.dev.yml` to
# every docker compose command below, and write `01flux-wa` wherever one names the `01flux-wa-api` service.
    ...
    - name: Configure staging webhooks
      command: |
        curl -X POST 'http://staging-host:2785/api/sessions/{sessionId}/webhooks' \
          -H "X-API-Key: $STAGING_API_KEY" \
          -H 'Content-Type: application/json' \
          -d '{"url":"https://staging-webhook.example.com/01flux-wa","events":["message.received"]}'
...
**Fix (automatic for most deployments):** 01flux WA ships a guard migration (`NormalizeSynchronizeUuidColumns`, ordered before the first collision) that converts the affected `uuid` columns to `varchar` on the next boot. For small-to-medium databases this is transparent — upgrade and restart.
...
DATABASE_TYPE=postgres DATABASE_HOST=... DATABASE_USERNAME=... \
  DATABASE_PASSWORD=... DATABASE_NAME=01flux-wa npm run migration:run
docker compose up -d
```
