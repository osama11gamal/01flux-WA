...
(Add the 01flux WA repo as a devDependency or reference the types via a `paths` mapping. A standalone
`@01flux-wa/plugin-types` package is planned.)
...
`registerWebhook`, and `registerSearchProvider`. (A formal `@01flux-wa/plugin-types` package with the full
