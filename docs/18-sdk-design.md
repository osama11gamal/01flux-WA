...
01flux WA ships five official, hand-written client libraries for the REST API. They are not generated from an OpenAPI spec — each is written directly against the real API surface (paths, request DTOs, response shapes) and **unit-tested with a mocked HTTP transport that asserts on the exact request path, method, and body**, so drift in what an SDK _sends_ is caught at test time rather than in production. That mechanism says nothing about what an SDK expects _back_; see §18.6 for how response shapes are held to the contract.

| Language                | Package                                                                                | Install                              | Notes                                                                            |
| ----------------------- | -------------------------------------------------------------------------------------- | ------------------------------------ | -------------------------------------------------------------------------------- |
| JavaScript / TypeScript | [`@rmyndharis/01flux-wa`](https://www.npmjs.com/package/@rmyndharis/01flux-wa)               | `npm install @rmyndharis/01flux-wa`     | Dual ESM + CJS, bundled `.d.ts` types, Node 18+                                  |
| Python                  | [`rmyndharis-01flux-wa`](https://pypi.org/project/rmyndharis-01flux-wa/)                     | `pip install rmyndharis-01flux-wa`      | Synchronous (httpx), PEP 561 typed, Python 3.9+                                  |
| PHP                     | [`rmyndharis/01flux-wa`](https://packagist.org/packages/rmyndharis/01flux-wa)                | `composer require rmyndharis/01flux-wa` | Synchronous (Guzzle 7), PSR-4, PHP 8.1+                                          |
| Java                    | [`com.rmyndharis:01flux-wa`](https://central.sonatype.com/artifact/com.rmyndharis/01flux-wa) | Maven Central                        | Sync, `java.net.http`. See [`sdk/java/README.md`](../sdk/java/README.md).        |
| Go                      | [`github.com/rmyndharis/01flux-wa/sdk/go`](../sdk/go)                                     | `go get`                             | Stdlib-only, no third-party deps. See [`sdk/go/README.md`](../sdk/go/README.md). |

> The import names differ from the dist names where the ecosystem requires it. Python installs `rmyndharis-01flux-wa` but imports `01flux-wa`; the client class is `FluxWaClient` in JS/Python and `01flux WA\Client` in PHP.

### Design Principles

- **One client, fluent resources.** A single client object (`FluxWaClient` / `01flux WA\Client`) exposes every resource as a property — `client.messages.sendText(...)`, `client.sessions.start(...)`. All five SDKs expose the **same** resource surface; only the language idioms differ (camelCase methods + objects in JS/PHP/Java, snake_case methods + dicts in Python, exported fields + structs in Go).
...
The official JavaScript/TypeScript SDK is published as **`@rmyndharis/01flux-wa`**. It is a pure promise-based HTTP client: a single `FluxWaClient` exposes every API resource as a typed property. There is no event model — the SDK does not open WebSockets, emit events, or expose `client.on(...)`. To receive inbound messages and acks, configure a webhook (see the `webhooks` resource) and host your own HTTP receiver.
...
```bash
npm install @rmyndharis/01flux-wa
# or
yarn add @rmyndharis/01flux-wa
# or
pnpm add @rmyndharis/01flux-wa
```
...
```typescript
import { FluxWaClient } from '@rmyndharis/01flux-wa';
...
```javascript
const { FluxWaClient } = require('@rmyndharis/01flux-wa');
```

### Client Configuration

The constructor takes a single `FluxWaClientOptions` object. `baseUrl` and `apiKey` are required (the constructor throws synchronously if either is missing).

| Option           | Type                     | Required | Default            | Description                                                                                                                                                                    |
| ---------------- | ------------------------ | -------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `baseUrl`        | `string`                 | yes      | —                  | Base URL of the 01flux WA API, e.g. `http://localhost:2785`. A trailing slash is trimmed; a path prefix (e.g. `https://host/v1`) is preserved.                                    |
...
All resources are accessed as properties on the client (`client.<resource>.<method>`). Every method returns a `Promise`. Methods marked **OPERATOR** require an `OPERATOR`-level API key (an `ADMIN` key satisfies it); a `VIEWER` key receives a `403` (`FluxWaForbiddenError`).
...
On a non-2xx response the SDK throws a typed `FluxWaApiError` subclass carrying `.status` (HTTP status), `.body` (parsed JSON error envelope, or raw text), and `.errorKind` (the NestJS `error` field, `undefined` when the gateway omits it — which is the norm for a validation rejection in production, where `disableErrorMessages` is on). All error classes extend `FluxWaError` and are exported, so they are `instanceof`-checkable. A timeout throws `FluxWaTimeoutError`, which extends `FluxWaError` directly (not `FluxWaApiError`).

| Error class                     | HTTP status | Meaning                                                                                       |
| ------------------------------- | ----------- | --------------------------------------------------------------------------------------------- |
| `FluxWaAuthError`               | 401         | Missing or invalid API key.                                                                   |
| `FluxWaForbiddenError`          | 403         | The key's role is insufficient (e.g. an OPERATOR-only route).                                 |
| `FluxWaNotFoundError`           | 404         | Resource not found.                                                                           |
| `FluxWaConflictError`           | 409         | Conflict — typically the engine is not ready.                                                 |
| `FluxWaRateLimitError`          | 429         | Rate limited.                                                                                 |
| `FluxWaNotImplementedError`     | 501         | The active engine does not support this operation.                                            |
| `FluxWaServiceUnavailableError` | 503         | The engine did not confirm in time. The only retryable error here — 501 is permanent.         |
| `FluxWaApiError`                | any other   | Generic non-2xx (the base API error, e.g. `400`; also surfaced for unfollowed 3xx redirects). |
| `FluxWaTimeoutError`            | —           | The request exceeded the configured timeout.                                                  |
...
try {
  ...
  if (err instanceof FluxWaConflictError) {
    // 409 — engine not ready; start the session first.
  } else if (err instanceof FluxWaNotFoundError) {
    // 404 — session/chat does not exist.
  } else if (err instanceof FluxWaRateLimitError) {
    // 429 — back off and retry.
  } else if (err instanceof FluxWaTimeoutError) {
    // request timed out.
  } else if (err instanceof FluxWaApiError) {
...
- **Redirects are never followed.** The transport uses `redirect: 'manual'`, so a `3xx` surfaces to the caller as an error (via `FluxWaApiError`) rather than being followed — this guarantees the `X-API-Key` header is never re-sent to a redirect target (potentially a different origin).
...
- **Distribution (PyPI):** `rmyndharis-01flux-wa`
- **Import package:** `01flux-wa`
- **Client class:** `FluxWaClient`
- **Python:** `>=3.9` (per `pyproject.toml`)
- **Runtime dependency:** `httpx>=0.25.0,<1.0`
- **Typed:** ships `py.typed` markers for `01flux-wa` and `01flux-wa.resources` (PEP 561)
...
`FluxWaClient` is also a context manager, so the connection pool is closed for you:

```python
with FluxWaClient(base_url="http://localhost:2785", api_key="flx_k1_…") as client:
...
| `timeout`         | `float`                       | `30.0`       | Per-request timeout in seconds. A breach raises `FluxWaTimeoutError`.                                                                   |
...
Every error inherits from `FluxWaError`. A non-2xx response raises an `FluxWaApiError` (or a more specific subclass picked by status); a timeout raises `FluxWaTimeoutError`. The API-error classes carry `.status` (HTTP code), `.body` (parsed JSON or raw text), and `.error_kind` (the NestJS `error` field).

| Exception                       | Trigger                                                  |
| ------------------------------- | -------------------------------------------------------- |
| `FluxWaAuthError`               | HTTP `401` — missing or invalid API key                  |
| `FluxWaForbiddenError`          | HTTP `403` — insufficient role                           |
| `FluxWaNotFoundError`           | HTTP `404` — resource not found                          |
| `FluxWaConflictError`           | HTTP `409` — typically engine-not-ready                  |
| `FluxWaRateLimitError`          | HTTP `429` — too many requests                           |
| `FluxWaNotImplementedError`     | HTTP `501` — active engine doesn't support the operation |
| `FluxWaServiceUnavailableError` | HTTP `503` — engine did not confirm in time; retryable   |
| `FluxWaApiError`                | any other non-2xx status (incl. unfollowed `3xx`)        |
| `FluxWaTimeoutError`            | request exceeded `timeout` (has a `.timeout` attribute)  |
...
client = FluxWaClient(base_url="http://localhost:2785", api_key="flx_k1_…")
...
except FluxWaConflictError:
    print("Session engine not ready yet — start it first.")
except FluxWaNotFoundError:
    print("Session does not exist.")
except FluxWaRateLimitError:
    print("Rate limited — back off and retry.")
except FluxWaTimeoutError as e:
    print(f"Timed out after {e.timeout}s")
except FluxWaApiError as e:
    print(f"API error {e.status} ({e.error_kind}): {e.body}")
```

### Notable Behaviors

- **Redirects are never followed.** `follow_redirects` is forced off so the `X-API-Key` header is never re-sent to a redirect target. An unfollowed `3xx` therefore surfaces as an `FluxWaApiError` rather than a success.
...
The PHP SDK is a hand-written, synchronous client built on Guzzle 7. It mirrors the full user-facing API surface: every resource method maps to one REST endpoint, request/response payloads are plain associative arrays, and non-2xx responses are translated into a typed `01flux WA*Exception` hierarchy.

### Installation

```bash
composer require rmyndharis/01flux-wa
```
...
- PSR-4 autoloaded under the `01flux WA\` namespace (`"01flux WA\\": "src/"`).
...
Missing `baseUrl` or `apiKey` throws `01flux WA\Exceptions\FluxWaException` from the constructor.
...
All exceptions live in `01flux WA\Exceptions` and descend from `FluxWaException` (which extends PHP's `\Exception`). Any non-2xx response is raised as an `FluxWaApiException`; the static `classify()` factory picks the most specific subclass by status code. An `FluxWaApiException` carries the HTTP status (`getStatus(): int`), the parsed error body (`getBody(): mixed`), and the NestJS `error` kind when present (`getErrorKind(): ?string`).

| Exception                           | Extends              | Trigger                                                                                      |
| ----------------------------------- | -------------------- | -------------------------------------------------------------------------------------------- |
| `FluxWaException`                   | `\Exception`         | Base for all SDK errors (also thrown for missing `baseUrl`/`apiKey`).                        |
| `FluxWaApiException`                | `FluxWaException`    | Any non-2xx (including unfollowed 3xx and other 4xx/5xx).                                    |
| `FluxWaAuthException`               | `FluxWaApiException` | `401` — missing/invalid API key.                                                             |
| `FluxWaForbiddenException`          | `FluxWaApiException` | `403` — insufficient role (e.g. operator-only endpoint).                                     |
| `FluxWaNotFoundException`           | `FluxWaApiException` | `404` — resource not found.                                                                  |
| `FluxWaConflictException`           | `FluxWaApiException` | `409` — conflict (e.g. engine not ready).                                                    |
| `FluxWaRateLimitException`          | `FluxWaApiException` | `429` — rate limited.                                                                        |
| `FluxWaNotImplementedException`     | `FluxWaApiException` | `501` — active engine does not support the operation.                                        |
| `FluxWaServiceUnavailableException` | `FluxWaApiException` | `503` — engine did not confirm in time; the only retryable one.                              |
| `FluxWaTimeoutException`            | `FluxWaException`    | Request exceeded the timeout (`getTimeout(): float`). Not an API error — has no status/body. |

```php
<?php
use 01flux WA\Client;
use 01flux WA\Exceptions\FluxWaConflictException;
use 01flux WA\Exceptions\FluxWaRateLimitException;
use 01flux WA\Exceptions\FluxWaTimeoutException;
use 01flux WA\Exceptions\FluxWaApiException;
...
} catch (FluxWaConflictException $e) {
    // 409 — engine not ready yet
} catch (FluxWaRateLimitException $e) {
    // 429 — back off and retry yourself (no auto-retry)
} catch (FluxWaTimeoutException $e) {
    fwrite(STDERR, "timed out after {$e->getTimeout()}s\n");
} catch (FluxWaApiException $e) {
    // any other non-2xx
...
- **Redirects are never followed.** Guzzle is configured with `allow_redirects => false`, so a `3xx` surfaces as an `FluxWaApiException` rather than being followed — the `X-API-Key` header is never re-sent to a redirect target.
...
- **PSR-4 autoloading.** Everything lives under the `01flux WA\` namespace mapped to `src/`; `composer require rmyndharis/01flux-wa` wires up the autoloader.

## 18.5 n8n Community Node

01flux WA's n8n integration is **not** part of these SDK packages. It is a separate community node maintained in its own repository, which speaks the same REST + webhook contract documented in [API Specification](./06-api-specification.md):

- An **action/HTTP** path that calls the REST endpoints (e.g. `POST /api/sessions/:id/messages/send-text`) with the `X-API-Key` header.
- A **trigger** path that registers a webhook (the `webhooks` resource) and receives inbound events, verifying the `X-FluxWa-Signature` HMAC.
...
| --------------------- | -------------------- | ------------------------------------- |
| JavaScript/TypeScript | npm                  | `@rmyndharis/01flux-wa`                  |
| Python                | PyPI                 | `rmyndharis-01flux-wa`                   |
| PHP                   | Packagist            | `rmyndharis/01flux-wa`                   |
| Java                  | Maven Central        | `com.rmyndharis:01flux-wa`               |
| Go                    | (none — module path) | `github.com/rmyndharis/01flux-wa/sdk/go` |
