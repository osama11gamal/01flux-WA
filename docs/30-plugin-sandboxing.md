...
01flux WA runs **untrusted plugins** (anything loaded from the plugins directory) in an isolated worker
...
as the same uid as 01flux WA. The sandbox therefore does **not**, by itself, stop a malicious plugin
from reading files the 01flux WA process can read or making outbound network connections. It protects
...
- **Run 01flux WA in a container.** The image's entrypoint already drops to the non-root `01flux-wa` user
