#!/bin/sh
# Smoke test: verify the built image runs its process as the fluxwa user (not root).
...
# CI already builds the image; set FLUX_WA_SMOKE_IMAGE to reuse that tag instead of paying for a
# second build. Unset (the local default) keeps the original behaviour: build here, remove after.
IMAGE_TAG="${FLUX_WA_SMOKE_IMAGE:-01flux-wa-test-non-root:smoke}"
BUILT_HERE=0

if [ -z "${FLUX_WA_SMOKE_IMAGE:-}" ]; then
  BUILT_HERE=1
...
# Override CMD with 'id' so docker-entrypoint.sh runs: exec gosu fluxwa id
...
if echo "$USER_OUTPUT" | grep -q "01flux-wa"; then
  echo "PASS: process runs as 01flux-wa (non-root)"
else
  echo "FAIL: process is not running as the fluxwa user" >&2
  cleanup
  exit 1
fi
