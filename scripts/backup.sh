...
#   - plugin-state/    — registry and persisted ctx.storage state under FLUX_WA_DATA_DIR
#   - .env.generated and .api-key — dashboard config and plaintext bootstrap admin key
#
# The previous runbook backed up the wrong file (01flux-wa.db) and omitted main.sqlite,
...
#                       lib-env.sh). They are NOT derived from FLUX_WA_DATA_DIR — the app never does
#                       that either.
#   FLUX_WA_DATA_DIR   data directory for the non-DB state below (default: ./data)
...
# FLUX_WA_DATA_DIR and BACKUP_DIR steer the script itself and are never written to an env file, so
...
DATA_DIR="${FLUX_WA_DATA_DIR:-./data}"
BACKUP_DIR="${BACKUP_DIR:-./backups}"
# shellcheck source=scripts/lib-env.sh
. "$(dirname "$0")/lib-env.sh"
DATABASE_TYPE="$(flux_wa_resolve DATABASE_TYPE sqlite)"
TIMESTAMP="$(date +%Y%m%d-%H%M%S)"

# Database paths resolve exactly like the app: an explicit environment value wins, then ./.env, then
# the dashboard's <data dir>/.env.generated, otherwise the fixed ./data default. FLUX_WA_DATA_DIR
# below only bases the non-DB state directories — deriving DB paths from it would back up files the
# app never reads.
MAIN_DB="$(flux_wa_resolve MAIN_DATABASE_NAME ./data/main.sqlite)"
DATA_DB="$(flux_wa_resolve DATABASE_NAME ./data/openwa.sqlite)"
SESSIONS_DIR="$(flux_wa_resolve SESSION_DATA_PATH "$DATA_DIR/sessions")"
BAILEYS_DIR="$(flux_wa_resolve BAILEYS_AUTH_DIR "$DATA_DIR/baileys")"
MEDIA_DIR="$(flux_wa_resolve STORAGE_LOCAL_PATH "$DATA_DIR/media")"
...
PLUGIN_PACKAGES_DIR="$(flux_wa_resolve PLUGINS_DIR "$DATA_DIR/plugins")"
...
PLUGIN_STATE_ROOT="$(flux_wa_resolve PLUGIN_STATE_DIR "$DATA_DIR")"
...
if [ "$DATABASE_TYPE" = "postgres" ]; then
  ...
  DATABASE_URL_RESOLVED="$(flux_wa_resolve DATABASE_URL '')"
  if [ -n "$DATABASE_URL_RESOLVED" ]; then
    ...
    PGPASSWORD="$(flux_wa_resolve DATABASE_PASSWORD '')" pg_dump \
      -h "$(flux_wa_resolve DATABASE_HOST localhost)" \
      -p "$(flux_wa_resolve DATABASE_PORT 5432)" \
      -U "$(flux_wa_resolve DATABASE_USERNAME 01flux-wa)" \
      "$(flux_wa_resolve DATABASE_NAME 01flux-wa)" >"$STAGE/database.sql"
  fi
...
mkdir -p "$BACKUP_DIR"
ARCHIVE="$BACKUP_DIR/01flux-wa-backup-$TIMESTAMP.tar.gz"
