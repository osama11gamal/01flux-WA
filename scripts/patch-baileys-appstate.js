 ...
 * anyway (01flux WA calls it on every `open`).
 *
...
const LOOP_REPLACE = `                const decoded = await extractSyncdPatches(result, config?.options);
                // 01flux WA: query() resolves undefined on its own timeout, which decodes to {}. Every
...
function applyAppStatePatch(baileysDir = DEFAULT_BAILEYS) {
  ...
  if (source.includes('01flux WA: query() resolves undefined on its own timeout')) {
