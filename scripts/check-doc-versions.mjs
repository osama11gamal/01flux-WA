 ...
 *   6. charts/01flux-wa/Chart.yaml `appVersion` must equal package.json — the chart defaults its image
...
{
  const chart = read('charts/01flux-wa/Chart.yaml');
  const appVersion = chart.match(/^appVersion:\s*["']?([^"'\s]+)["']?\s*$/m)?.[1];
  if (!appVersion) {
    errors.push('charts/01flux-wa/Chart.yaml: no appVersion line found — it must name the shipped application version.');
  } else if (appVersion !== version) {
    errors.push(
      `charts/01flux-wa/Chart.yaml: appVersion is ${appVersion} but package.json is ${version} — the chart would deploy an image tag that does not exist yet.`,
    );
  }
}
