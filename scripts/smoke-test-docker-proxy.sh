#!/bin/sh
# Smoke test: verify 01flux-wa-api can list containers via docker-socket-proxy.
...
echo "==> Checking 01flux-wa-api health..."
...
PROXY_STATE=$(docker inspect --format='{{.State.Status}}' 01flux-wa-docker-proxy 2>/dev/null || echo "not_found")
if [ "$PROXY_STATE" != "running" ]; then
  echo "FAIL: 01flux-wa-docker-proxy is not running (state: $PROXY_STATE)" >&2
  exit 1
fi
echo "PASS: 01flux-wa-docker-proxy is running"

echo ""
echo "==> Verifying the proxy permits the read operations orchestration needs (from fluxwa-api)..."
# 01flux-wa-api is the only container that can reach docker-proxy:2375 (internal network), so
...
for endpoint in _ping containers/json; do
  CODE=$(docker exec 01flux-wa-api node -e "fetch('http://docker-proxy:2375/$endpoint').then(r=>console.log(r.status)).catch(()=>console.log(0))" 2>/dev/null || echo 0)
  if [ "$CODE" != "200" ]; then
...
CODE=$(docker exec 01flux-wa-api node -e "fetch('http://docker-proxy:2375/networks').then(r=>console.log(r.status)).catch(()=>console.log(0))" 2>/dev/null || echo 0)
if [ "$CODE" != "403" ]; then
...
# configuration. 01flux WA itself never issues deletes (profile teardown is stop-only);
# see SECURITY.md "Docker socket proxy — scope and residual risk".

echo ""
echo "==> Verifying 01flux-wa-api socket mount is gone..."
SOCKET_MOUNT=$(docker inspect 01flux-wa-api --format='{{range .Mounts}}{{.Source}}{{"\n"}}{{end}}' 2>/dev/null | grep "docker.sock" || true)
if [ -n "$SOCKET_MOUNT" ]; then
  echo "FAIL: 01flux-wa-api still has a docker.sock mount: $SOCKET_MOUNT" >&2
  exit 1
fi
echo "PASS: 01flux-wa-api has no direct docker.sock mount"
