 ...
 * name). whatsapp-web.js 1.34.7 (what 01flux WA pins) reads `_serialized` in the
 ...
 * `Base._normalizeId` (i.e. upstream shipped #201832 and 01flux WA bumped its dep).
 *
 ...
 *
 * GIT_CEILING_DIRECTORIES is what makes it work at all: without it git discovers the parent 01flux WA
...
function applyBackport(wwjsDir = DEFAULT_WWJS, patchFile = DEFAULT_PATCH) {
  ...
  // helper: Message.js is the load-bearing site (01flux WA reads `msg.id._serialized` in ~40 places),
