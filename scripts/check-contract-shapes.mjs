...
/** Python client pairs (TypedDict classes in 01flux-wa/types.py). */
...
/**
 * Python (sdk/python/fluxwa/types.py): TypedDict classes with `total=False` optionality,
...
if (isDirectRun) {
  ...
  const javaSources = () =>
    readdirSync(`${REPO_ROOT}sdk/java/src/main/java/com/rmyndharis/fluxwa/model`)
      .filter(f => f.endsWith('.java'))
      .map(f => readFileSync(`${REPO_ROOT}sdk/java/src/main/java/com/rmyndharis/fluxwa/model/${f}`, 'utf8'));
  ...
  const CLIENTS = [
    ...
    {
      label: 'sdk/python/fluxwa/types.py',
      load: () => parsePythonTypes(readFileSync(`${REPO_ROOT}sdk/python/fluxwa/types.py`, 'utf8')),
      mapping: PYTHON_MAPPING,
      excluded: EXCLUDED['sdk/python/fluxwa/types.py'] ?? {},
      floor: MINIMUM_MAPPED['sdk/python/fluxwa/types.py'],
      compareOptionality: true,
    },
    {
