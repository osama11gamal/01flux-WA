...
(
  ...
  MAIN_DATABASE_NAME="$A/custom/auth.sqlite" \
    ...
    FLUX_WA_DATA_DIR="$A/state" \
    BACKUP_DIR="$A/out" \
    "$BACKUP" >/dev/null
)
ARCHIVE_A="$(ls "$A"/out/01flux-wa-backup-*.tar.gz)"
...
(
  ...
  MAIN_DATABASE_NAME="$A/restore/custom-main.sqlite" \
    ...
    FLUX_WA_DATA_DIR="$A/restore/state" \
...
mkdir -p "$B"
set +e
OUT_B="$(cd "$B" && FLUX_WA_DATA_DIR="$B/state" BACKUP_DIR="$B/out" "$BACKUP" 2>&1)"
RC_B=$?
set -e
if [ "$RC_B" -eq 0 ]; then
...
if [ "$HAS_SQLITE3" -eq 1 ]; then
  ...
  ARCHIVE_C="$(ls "$C"/out/01flux-wa-backup-*.tar.gz)"
...
ARCHIVE_D="$(ls "$D"/out/01flux-wa-backup-*.tar.gz)"
...
(
  cd "$F"
  FLUX_WA_DATA_DIR="$F/state" BACKUP_DIR="$F/out" "$BACKUP" >/dev/null
)
ARCHIVE_F="$(ls "$F"/out/01flux-wa-backup-*.tar.gz)"
...
(
  cd "$F/restore"
  FLUX_WA_DATA_DIR="$F/restore/state" "$RESTORE" "$ARCHIVE_F" >/dev/null
)
...
(
  ...
  MAIN_DATABASE_NAME="$F/data/main.sqlite" DATABASE_NAME="$F/data/openwa.sqlite" \
    FLUX_WA_DATA_DIR="$F/state" BACKUP_DIR="$F/out2" "$BACKUP" >/dev/null
)
rm -rf "${F:?}/extract2" && mkdir -p "$F/extract2"
tar -xzf "$(ls "$F"/out2/01flux-wa-backup-*.tar.gz)" -C "$F/extract2"
...
# $FLUX_WA_DATA_DIR/plugins, so with the knob set the archive carried neither the registry nor any
...
(
  cd "$G"
  FLUX_WA_DATA_DIR="$G/state" PLUGIN_STATE_DIR="$G/elsewhere" BACKUP_DIR="$G/out" \
    MAIN_DATABASE_NAME="$G/state/main.sqlite" DATABASE_NAME="$G/state/openwa.sqlite" "$BACKUP" >/dev/null
)
ARCHIVE_G="$(ls "$G"/out/01flux-wa-backup-*.tar.gz)"
...
(
  cd "$G"
  FLUX_WA_DATA_DIR="$G/restore/state" PLUGIN_STATE_DIR="$G/restored-elsewhere" \
