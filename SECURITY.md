...
01flux WA is a self-hosted WhatsApp API gateway. It handles API-key authentication,
...
01flux WA already ships several hardening measures: API-key auth with roles
...
When exposing 01flux WA, please review the security-relevant configuration documented in
...
Installing or enabling a plugin is executing third-party code on the host that runs 01flux WA.
...
01flux WA process — run them in a separate container/VM with an OS-level sandbox and reach
01flux WA over the API like any other client.
...
Mitigations in place: the proxy is unreachable except from `01flux-wa-api` (dedicated
...
and 01flux WA itself never issues deletes (profile teardown is stop-only). If you do not
